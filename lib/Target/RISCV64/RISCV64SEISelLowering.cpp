//===- RISCV64SEISelLowering.cpp - RISCV64SE DAG Lowering Interface -------------===//
//
...
//===----------------------------------------------------------------------===//
//
// Subclass of RISCV64TargetLowering specialized for mips32/64.
//
//===----------------------------------------------------------------------===//

#include "RISCV64SEISelLowering.h"
...
static cl::opt<bool>
UseRISCV64TailCalls("mips-tail-calls", cl::Hidden,
...
RISCV64SETargetLowering::RISCV64SETargetLowering(const RISCV64TargetMachine &TM,
                                           const RISCV64Subtarget &STI)
    : RISCV64TargetLowering(TM, STI) {
  // Set up the register classes
  addRegisterClass(MVT::i32, &RISCV64::GPR32RegClass);

  if (Subtarget.isGP64bit())
    addRegisterClass(MVT::i64, &RISCV64::GPR64RegClass);
  ...
  if (Subtarget.hasDSP()) {
    ...
    for (unsigned i = 0; i < array_lengthof(VecTys); ++i) {
      addRegisterClass(VecTys[i], &RISCV64::DSPRRegClass);
  ...
  if (Subtarget.hasMSA()) {
    addMSAIntType(MVT::v16i8, &RISCV64::MSA128BRegClass);
    addMSAIntType(MVT::v8i16, &RISCV64::MSA128HRegClass);
    addMSAIntType(MVT::v4i32, &RISCV64::MSA128WRegClass);
    addMSAIntType(MVT::v2i64, &RISCV64::MSA128DRegClass);
    addMSAFloatType(MVT::v8f16, &RISCV64::MSA128HRegClass);
    addMSAFloatType(MVT::v4f32, &RISCV64::MSA128WRegClass);
    addMSAFloatType(MVT::v2f64, &RISCV64::MSA128DRegClass);

    // f16 is a storage-only type, always promote it to f32.
    addRegisterClass(MVT::f16, &RISCV64::MSA128HRegClass);
  ...
  if (!Subtarget.useSoftFloat()) {
    addRegisterClass(MVT::f32, &RISCV64::FGR32RegClass);

    // When dealing with single precision only, use libcalls
    if (!Subtarget.isSingleFloat()) {
      if (Subtarget.isFP64bit())
        addRegisterClass(MVT::f64, &RISCV64::FGR64RegClass);
      else
        addRegisterClass(MVT::f64, &RISCV64::AFGR64RegClass);
    }
  }
...
llvm::createRISCV64SETargetLowering(const RISCV64TargetMachine &TM,
                                 const RISCV64Subtarget &STI) {
  return new RISCV64SETargetLowering(TM, STI);
}

const TargetRegisterClass *
RISCV64SETargetLowering::getRepRegClassFor(MVT VT) const {
  if (VT == MVT::Untyped)
    return Subtarget.hasDSP() ? &RISCV64::ACC64DSPRegClass : &RISCV64::ACC64RegClass;

  return TargetLowering::getRepRegClassFor(VT);
}

// Enable MSA support for the given integer type and Register class.
void RISCV64SETargetLowering::
...
SDValue RISCV64SETargetLowering::lowerSELECT(SDValue Op, SelectionDAG &DAG) const {
  if(!Subtarget.hasRISCV6432r6())
    return RISCV64TargetLowering::LowerOperation(Op, DAG);
  ...
  SDValue Tmp = DAG.getNode(RISCV64ISD::MTC1_D64, DL, MVT::f64, Op->getOperand(0));
  return DAG.getNode(RISCV64ISD::FSELECT, DL, ResTy, Tmp, Op->getOperand(1),
                     Op->getOperand(2));
}

bool
RISCV64SETargetLowering::allowsMisalignedMemoryAccesses(EVT VT,
                                                     unsigned,
                                                     unsigned,
                                                     bool *Fast) const {
...
SDValue RISCV64SETargetLowering::LowerOperation(SDValue Op,
                                             SelectionDAG &DAG) const {
  switch(Op.getOpcode()) {
  case ISD::LOAD:  return lowerLOAD(Op, DAG);
  case ISD::STORE: return lowerSTORE(Op, DAG);
  case ISD::SMUL_LOHI: return lowerMulDiv(Op, RISCV64ISD::Mult, true, true, DAG);
  case ISD::UMUL_LOHI: return lowerMulDiv(Op, RISCV64ISD::Multu, true, true, DAG);
  case ISD::MULHS:     return lowerMulDiv(Op, RISCV64ISD::Mult, false, true, DAG);
  case ISD::MULHU:     return lowerMulDiv(Op, RISCV64ISD::Multu, false, true, DAG);
  case ISD::MUL:       return lowerMulDiv(Op, RISCV64ISD::Mult, true, false, DAG);
  case ISD::SDIVREM:   return lowerMulDiv(Op, RISCV64ISD::DivRem, true, true, DAG);
  case ISD::UDIVREM:   return lowerMulDiv(Op, RISCV64ISD::DivRemU, true, true,
                                          DAG);
...
// Fold zero extensions into RISCV64ISD::VEXTRACT_[SZ]EXT_ELT
//
// Performs the following transformations:
// - Changes RISCV64ISD::VEXTRACT_[SZ]EXT_ELT to zero extension if its
...
static SDValue performANDCombine(SDNode *N, SelectionDAG &DAG,
  ...
  // (and (RISCV64VExtract[SZ]Ext $a, $b, $c), imm:$d)
  // where $d + 1 == 2^n and n == 32
  // or    $d + 1 == 2^n and n <= 32 and ZExt
  // -> (RISCV64VExtractZExt $a, $b, $c)
  if (Op0Opcode == RISCV64ISD::VEXTRACT_SEXT_ELT ||
      Op0Opcode == RISCV64ISD::VEXTRACT_ZEXT_ELT) {
    ...
    if ((Op0Opcode == RISCV64ISD::VEXTRACT_ZEXT_ELT && Log2 >= ExtendTySize) ||
        Log2 == ExtendTySize) {
      SDValue Ops[] = { Op0->getOperand(0), Op0->getOperand(1), Op0Op2 };
      return DAG.getNode(RISCV64ISD::VEXTRACT_ZEXT_ELT, SDLoc(Op0),
                         Op0->getVTList(),
...
//
// This function is quite similar to RISCV64SEDAGToDAGISel::selectVSplat. The
...
static SDValue performMULCombine(SDNode *N, SelectionDAG &DAG,
                                 ...
                                 const RISCV64SETargetLowering *TL,
                                 const RISCV64Subtarget &Subtarget) {
...
static SDValue performSHLCombine(SDNode *N, SelectionDAG &DAG,
  ...
  return performDSPShiftCombine(RISCV64ISD::SHLL_DSP, N, Ty, DAG, Subtarget);
}

// Fold sign-extensions into RISCV64ISD::VEXTRACT_[SZ]EXT_ELT for MSA and fold
// constant splats into RISCV64ISD::SHRA_DSP for DSPr2.
//
// Performs the following transformations:
// - Changes RISCV64ISD::VEXTRACT_[SZ]EXT_ELT to sign extension if its
//   sign/zero-extension is completely overwritten by the new one performed by
...
static SDValue performSRACombine(SDNode *N, SelectionDAG &DAG,
  ...
  if (Subtarget.hasMSA()) {
    ...
    // (sra (shl (RISCV64VExtract[SZ]Ext $a, $b, $c), imm:$d), imm:$d)
    // where $d + sizeof($c) == 32
    // or    $d + sizeof($c) <= 32 and SExt
    // -> (RISCV64VExtractSExt $a, $b, $c)
    if (Op0->getOpcode() == ISD::SHL && Op1 == Op0->getOperand(1)) {
      ...
      if (Op0Op0->getOpcode() != RISCV64ISD::VEXTRACT_SEXT_ELT &&
          Op0Op0->getOpcode() != RISCV64ISD::VEXTRACT_ZEXT_ELT)
        return SDValue();
      ...
      if (TotalBits == 32 ||
          (Op0Op0->getOpcode() == RISCV64ISD::VEXTRACT_SEXT_ELT &&
           TotalBits <= 32)) {
        SDValue Ops[] = { Op0Op0->getOperand(0), Op0Op0->getOperand(1),
                          Op0Op0->getOperand(2) };
        return DAG.getNode(RISCV64ISD::VEXTRACT_SEXT_ELT, SDLoc(Op0Op0),
                           Op0Op0->getVTList(),
  ...
  return performDSPShiftCombine(RISCV64ISD::SHRA_DSP, N, Ty, DAG, Subtarget);
}
...
static SDValue performSRLCombine(SDNode *N, SelectionDAG &DAG,
  ...
  return performDSPShiftCombine(RISCV64ISD::SHRL_DSP, N, Ty, DAG, Subtarget);
}
...
static SDValue performSETCCCombine(SDNode *N, SelectionDAG &DAG) {
  ...
  return DAG.getNode(RISCV64ISD::SETCC_DSP, SDLoc(N), Ty, N->getOperand(0),
...
static SDValue performVSELECTCombine(SDNode *N, SelectionDAG &DAG) {
  ...
  if (Ty == MVT::v2i16 || Ty == MVT::v4i8) {
    ...
    if (SetCC.getOpcode() != RISCV64ISD::SETCC_DSP)
      return SDValue();

    return DAG.getNode(RISCV64ISD::SELECT_CC_DSP, SDLoc(N), Ty,
...
static SDValue performXORCombine(SDNode *N, SelectionDAG &DAG,
  ...
  if (Subtarget.hasMSA() && Ty.is128BitVector() && Ty.isInteger()) {
    ...
    if (NotOp->getOpcode() == ISD::OR)
      return DAG.getNode(RISCV64ISD::VNOR, SDLoc(N), Ty, NotOp->getOperand(0),
                         NotOp->getOperand(1));
  }
...
SDValue
RISCV64SETargetLowering::PerformDAGCombine(SDNode *N, DAGCombinerInfo &DCI) const {
  ...
  if (Val.getNode()) {
    LLVM_DEBUG(dbgs() << "\nRISCV64SE DAG Combine:\n";
               N->printrWithDepth(dbgs(), &DAG); dbgs() << "\n=> \n";
               Val.getNode()->printrWithDepth(dbgs(), &DAG); dbgs() << "\n");
    return Val;
  }

  return RISCV64TargetLowering::PerformDAGCombine(N, DCI);
}

MachineBasicBlock *
RISCV64SETargetLowering::EmitInstrWithCustomInserter(MachineInstr &MI,
                                                  MachineBasicBlock *BB) const {
  switch (MI.getOpcode()) {
  default:
    return RISCV64TargetLowering::EmitInstrWithCustomInserter(MI, BB);
  case RISCV64::BPOSGE32_PSEUDO:
    return emitBPOSGE32(MI, BB);
  case RISCV64::SNZ_B_PSEUDO:
    return emitMSACBranchPseudo(MI, BB, RISCV64::BNZ_B);
  case RISCV64::SNZ_H_PSEUDO:
    return emitMSACBranchPseudo(MI, BB, RISCV64::BNZ_H);
  case RISCV64::SNZ_W_PSEUDO:
    return emitMSACBranchPseudo(MI, BB, RISCV64::BNZ_W);
  case RISCV64::SNZ_D_PSEUDO:
    return emitMSACBranchPseudo(MI, BB, RISCV64::BNZ_D);
  case RISCV64::SNZ_V_PSEUDO:
    return emitMSACBranchPseudo(MI, BB, RISCV64::BNZ_V);
  case RISCV64::SZ_B_PSEUDO:
    return emitMSACBranchPseudo(MI, BB, RISCV64::BZ_B);
  case RISCV64::SZ_H_PSEUDO:
    return emitMSACBranchPseudo(MI, BB, RISCV64::BZ_H);
  case RISCV64::SZ_W_PSEUDO:
    return emitMSACBranchPseudo(MI, BB, RISCV64::BZ_W);
  case RISCV64::SZ_D_PSEUDO:
    return emitMSACBranchPseudo(MI, BB, RISCV64::BZ_D);
  case RISCV64::SZ_V_PSEUDO:
    return emitMSACBranchPseudo(MI, BB, RISCV64::BZ_V);
  case RISCV64::COPY_FW_PSEUDO:
    return emitCOPY_FW(MI, BB);
  case RISCV64::COPY_FD_PSEUDO:
    return emitCOPY_FD(MI, BB);
  case RISCV64::INSERT_FW_PSEUDO:
    return emitINSERT_FW(MI, BB);
  case RISCV64::INSERT_FD_PSEUDO:
    return emitINSERT_FD(MI, BB);
  case RISCV64::INSERT_B_VIDX_PSEUDO:
  case RISCV64::INSERT_B_VIDX64_PSEUDO:
    return emitINSERT_DF_VIDX(MI, BB, 1, false);
  case RISCV64::INSERT_H_VIDX_PSEUDO:
  case RISCV64::INSERT_H_VIDX64_PSEUDO:
    return emitINSERT_DF_VIDX(MI, BB, 2, false);
  case RISCV64::INSERT_W_VIDX_PSEUDO:
  case RISCV64::INSERT_W_VIDX64_PSEUDO:
    return emitINSERT_DF_VIDX(MI, BB, 4, false);
  case RISCV64::INSERT_D_VIDX_PSEUDO:
  case RISCV64::INSERT_D_VIDX64_PSEUDO:
    return emitINSERT_DF_VIDX(MI, BB, 8, false);
  case RISCV64::INSERT_FW_VIDX_PSEUDO:
  case RISCV64::INSERT_FW_VIDX64_PSEUDO:
    return emitINSERT_DF_VIDX(MI, BB, 4, true);
  case RISCV64::INSERT_FD_VIDX_PSEUDO:
  case RISCV64::INSERT_FD_VIDX64_PSEUDO:
    return emitINSERT_DF_VIDX(MI, BB, 8, true);
  case RISCV64::FILL_FW_PSEUDO:
    return emitFILL_FW(MI, BB);
  case RISCV64::FILL_FD_PSEUDO:
    return emitFILL_FD(MI, BB);
  case RISCV64::FEXP2_W_1_PSEUDO:
    return emitFEXP2_W_1(MI, BB);
  case RISCV64::FEXP2_D_1_PSEUDO:
    return emitFEXP2_D_1(MI, BB);
  case RISCV64::ST_F16:
    return emitST_F16_PSEUDO(MI, BB);
  case RISCV64::LD_F16:
    return emitLD_F16_PSEUDO(MI, BB);
  case RISCV64::MSA_FP_EXTEND_W_PSEUDO:
    return emitFPEXTEND_PSEUDO(MI, BB, false);
  case RISCV64::MSA_FP_ROUND_W_PSEUDO:
    return emitFPROUND_PSEUDO(MI, BB, false);
  case RISCV64::MSA_FP_EXTEND_D_PSEUDO:
    return emitFPEXTEND_PSEUDO(MI, BB, true);
  case RISCV64::MSA_FP_ROUND_D_PSEUDO:
    return emitFPROUND_PSEUDO(MI, BB, true);
  }
}

bool RISCV64SETargetLowering::isEligibleForTailCallOptimization(
    const CCState &CCInfo, unsigned NextStackOffset,
    const RISCV64FunctionInfo &FI) const {
  if (!UseRISCV64TailCalls)
    return false;
...
SDValue RISCV64SETargetLowering::lowerLOAD(SDValue Op, SelectionDAG &DAG) const {
  LoadSDNode &Nd = *cast<LoadSDNode>(Op);

  if (Nd.getMemoryVT() != MVT::f64 || !NoDPLoadStore)
    return RISCV64TargetLowering::lowerLOAD(Op, DAG);
  ...
  SDValue BP = DAG.getNode(RISCV64ISD::BuildPairF64, DL, MVT::f64, Lo, Hi);
  SDValue Ops[2] = {BP, Hi.getValue(1)};
  return DAG.getMergeValues(Ops, DL);
}

SDValue RISCV64SETargetLowering::lowerSTORE(SDValue Op, SelectionDAG &DAG) const {
  StoreSDNode &Nd = *cast<StoreSDNode>(Op);

  if (Nd.getMemoryVT() != MVT::f64 || !NoDPLoadStore)
    return RISCV64TargetLowering::lowerSTORE(Op, DAG);
...
SDValue RISCV64SETargetLowering::lowerMulDiv(SDValue Op, unsigned NewOpc,
                                          bool HasLo, bool HasHi,
                                          SelectionDAG &DAG) const {
  // MIPS32r6/MIPS64r6 removed accumulator based multiplies.
  assert(!Subtarget.hasRISCV6432r6());
  ...
  if (HasLo)
    Lo = DAG.getNode(RISCV64ISD::MFLO, DL, Ty, Mult);
  if (HasHi)
    Hi = DAG.getNode(RISCV64ISD::MFHI, DL, Ty, Mult);
...
static SDValue initAccumulator(SDValue In, const SDLoc &DL, SelectionDAG &DAG) {
  ...
  return DAG.getNode(RISCV64ISD::MTLOHI, DL, MVT::Untyped, InLo, InHi);
}

static SDValue extractLOHI(SDValue Op, const SDLoc &DL, SelectionDAG &DAG) {
  SDValue Lo = DAG.getNode(RISCV64ISD::MFLO, DL, MVT::i32, Op);
  SDValue Hi = DAG.getNode(RISCV64ISD::MFHI, DL, MVT::i32, Op);
...
SDValue RISCV64SETargetLowering::lowerINTRINSIC_WO_CHAIN(SDValue Op,
                                                      SelectionDAG &DAG) const {
  SDLoc DL(Op);
  ...
  case Intrinsic::mips_shilo:
    return lowerDSPIntr(Op, DAG, RISCV64ISD::SHILO);
  case Intrinsic::mips_dpau_h_qbl:
    return lowerDSPIntr(Op, DAG, RISCV64ISD::DPAU_H_QBL);
  case Intrinsic::mips_dpau_h_qbr:
    return lowerDSPIntr(Op, DAG, RISCV64ISD::DPAU_H_QBR);
  case Intrinsic::mips_dpsu_h_qbl:
    return lowerDSPIntr(Op, DAG, RISCV64ISD::DPSU_H_QBL);
  case Intrinsic::mips_dpsu_h_qbr:
    return lowerDSPIntr(Op, DAG, RISCV64ISD::DPSU_H_QBR);
  case Intrinsic::mips_dpa_w_ph:
    return lowerDSPIntr(Op, DAG, RISCV64ISD::DPA_W_PH);
  case Intrinsic::mips_dps_w_ph:
    return lowerDSPIntr(Op, DAG, RISCV64ISD::DPS_W_PH);
  case Intrinsic::mips_dpax_w_ph:
    return lowerDSPIntr(Op, DAG, RISCV64ISD::DPAX_W_PH);
  case Intrinsic::mips_dpsx_w_ph:
    return lowerDSPIntr(Op, DAG, RISCV64ISD::DPSX_W_PH);
  case Intrinsic::mips_mulsa_w_ph:
    return lowerDSPIntr(Op, DAG, RISCV64ISD::MULSA_W_PH);
  case Intrinsic::mips_mult:
    return lowerDSPIntr(Op, DAG, RISCV64ISD::Mult);
  case Intrinsic::mips_multu:
    return lowerDSPIntr(Op, DAG, RISCV64ISD::Multu);
  case Intrinsic::mips_madd:
    return lowerDSPIntr(Op, DAG, RISCV64ISD::MAdd);
  case Intrinsic::mips_maddu:
    return lowerDSPIntr(Op, DAG, RISCV64ISD::MAddu);
  case Intrinsic::mips_msub:
    return lowerDSPIntr(Op, DAG, RISCV64ISD::MSub);
  case Intrinsic::mips_msubu:
    return lowerDSPIntr(Op, DAG, RISCV64ISD::MSubu);
  ...
  case Intrinsic::mips_bnz_d:
    return DAG.getNode(RISCV64ISD::VALL_NONZERO, DL, Op->getValueType(0),
                       Op->getOperand(1));
  case Intrinsic::mips_bnz_v:
    return DAG.getNode(RISCV64ISD::VANY_NONZERO, DL, Op->getValueType(0),
                       Op->getOperand(1));
  ...
  case Intrinsic::mips_bz_d:
    return DAG.getNode(RISCV64ISD::VALL_ZERO, DL, Op->getValueType(0),
                       Op->getOperand(1));
  case Intrinsic::mips_bz_v:
    return DAG.getNode(RISCV64ISD::VANY_ZERO, DL, Op->getValueType(0),
                       Op->getOperand(1));
  ...
  case Intrinsic::mips_copy_s_w:
    return lowerMSACopyIntr(Op, DAG, RISCV64ISD::VEXTRACT_SEXT_ELT);
  case Intrinsic::mips_copy_s_d:
    if (Subtarget.hasRISCV6464())
      // Lower directly into VEXTRACT_SEXT_ELT since i64 is legal on RISCV6464.
      return lowerMSACopyIntr(Op, DAG, RISCV64ISD::VEXTRACT_SEXT_ELT);
    else {
  ...
  case Intrinsic::mips_copy_u_w:
    return lowerMSACopyIntr(Op, DAG, RISCV64ISD::VEXTRACT_ZEXT_ELT);
  case Intrinsic::mips_copy_u_d:
    if (Subtarget.hasRISCV6464())
      // Lower directly into VEXTRACT_ZEXT_ELT since i64 is legal on RISCV6464.
      return lowerMSACopyIntr(Op, DAG, RISCV64ISD::VEXTRACT_ZEXT_ELT);
    else {
      // Lower into the generic EXTRACT_VECTOR_ELT node and let the type
      // legalizer and EXTRACT_VECTOR_ELT lowering sort it out.
  ...
  case Intrinsic::mips_fmsub_d: {
    // TODO: If intrinsics have fast-math-flags, propagate them.
    return DAG.getNode(RISCV64ISD::FMS, SDLoc(Op), Op->getValueType(0),
                       Op->getOperand(1), Op->getOperand(2), Op->getOperand(3));
  }
  ...
  case Intrinsic::mips_insve_d: {
    ...
    if (Value < 0 || Value > Max)
      report_fatal_error("Immediate out of range");
    return DAG.getNode(RISCV64ISD::INSVE, DL, Op->getValueType(0),
                       Op->getOperand(1), Op->getOperand(2), Op->getOperand(3),
                       DAG.getConstant(0, DL, MVT::i32));
    }
  ...
  case Intrinsic::mips_pckev_d:
    return DAG.getNode(RISCV64ISD::PCKEV, DL, Op->getValueType(0),
                       Op->getOperand(1), Op->getOperand(2));
  case Intrinsic::mips_pckod_b:
  case Intrinsic::mips_pckod_h:
  case Intrinsic::mips_pckod_w:
  case Intrinsic::mips_pckod_d:
    return DAG.getNode(RISCV64ISD::PCKOD, DL, Op->getValueType(0),
                       Op->getOperand(1), Op->getOperand(2));
  ...
  case Intrinsic::mips_shf_w: {
    ...
    if (Value < 0 || Value > 255)
      report_fatal_error("Immediate out of range");
    return DAG.getNode(RISCV64ISD::SHF, DL, Op->getValueType(0),
                       Op->getOperand(2), Op->getOperand(1));
  }
  ...
  case Intrinsic::mips_splat_d:
    ...
    // Instead we lower to RISCV64ISD::VSHF and match from there.
    return DAG.getNode(RISCV64ISD::VSHF, DL, Op->getValueType(0),
  ...
  case Intrinsic::thread_pointer: {
    ...
    return DAG.getNode(RISCV64ISD::ThreadPointer, DL, PtrVT);
  }
  }
}
...
SDValue RISCV64SETargetLowering::lowerINTRINSIC_W_CHAIN(SDValue Op,
                                                     SelectionDAG &DAG) const {
  ...
  case Intrinsic::mips_extp:
    return lowerDSPIntr(Op, DAG, RISCV64ISD::EXTP);
  case Intrinsic::mips_extpdp:
    return lowerDSPIntr(Op, DAG, RISCV64ISD::EXTPDP);
  case Intrinsic::mips_extr_w:
    return lowerDSPIntr(Op, DAG, RISCV64ISD::EXTR_W);
  case Intrinsic::mips_extr_r_w:
    return lowerDSPIntr(Op, DAG, RISCV64ISD::EXTR_R_W);
  case Intrinsic::mips_extr_rs_w:
    return lowerDSPIntr(Op, DAG, RISCV64ISD::EXTR_RS_W);
  case Intrinsic::mips_extr_s_h:
    return lowerDSPIntr(Op, DAG, RISCV64ISD::EXTR_S_H);
  case Intrinsic::mips_mthlip:
    return lowerDSPIntr(Op, DAG, RISCV64ISD::MTHLIP);
  case Intrinsic::mips_mulsaq_s_w_ph:
    return lowerDSPIntr(Op, DAG, RISCV64ISD::MULSAQ_S_W_PH);
  case Intrinsic::mips_maq_s_w_phl:
    return lowerDSPIntr(Op, DAG, RISCV64ISD::MAQ_S_W_PHL);
  case Intrinsic::mips_maq_s_w_phr:
    return lowerDSPIntr(Op, DAG, RISCV64ISD::MAQ_S_W_PHR);
  case Intrinsic::mips_maq_sa_w_phl:
    return lowerDSPIntr(Op, DAG, RISCV64ISD::MAQ_SA_W_PHL);
  case Intrinsic::mips_maq_sa_w_phr:
    return lowerDSPIntr(Op, DAG, RISCV64ISD::MAQ_SA_W_PHR);
  case Intrinsic::mips_dpaq_s_w_ph:
    return lowerDSPIntr(Op, DAG, RISCV64ISD::DPAQ_S_W_PH);
  case Intrinsic::mips_dpsq_s_w_ph:
    return lowerDSPIntr(Op, DAG, RISCV64ISD::DPSQ_S_W_PH);
  case Intrinsic::mips_dpaq_sa_l_w:
    return lowerDSPIntr(Op, DAG, RISCV64ISD::DPAQ_SA_L_W);
  case Intrinsic::mips_dpsq_sa_l_w:
    return lowerDSPIntr(Op, DAG, RISCV64ISD::DPSQ_SA_L_W);
  case Intrinsic::mips_dpaqx_s_w_ph:
    return lowerDSPIntr(Op, DAG, RISCV64ISD::DPAQX_S_W_PH);
  case Intrinsic::mips_dpaqx_sa_w_ph:
    return lowerDSPIntr(Op, DAG, RISCV64ISD::DPAQX_SA_W_PH);
  case Intrinsic::mips_dpsqx_s_w_ph:
    return lowerDSPIntr(Op, DAG, RISCV64ISD::DPSQX_S_W_PH);
  case Intrinsic::mips_dpsqx_sa_w_ph:
    return lowerDSPIntr(Op, DAG, RISCV64ISD::DPSQX_SA_W_PH);
...
SDValue RISCV64SETargetLowering::lowerINTRINSIC_VOID(SDValue Op,
                                                  SelectionDAG &DAG) const {
  unsigned Intr = cast<ConstantSDNode>(Op->getOperand(1))->getZExtValue();
  switch (Intr) {
  default:
...
// Lower ISD::EXTRACT_VECTOR_ELT into RISCV64ISD::VEXTRACT_SEXT_ELT.
//
...
// process).
SDValue RISCV64SETargetLowering::
lowerEXTRACT_VECTOR_ELT(SDValue Op, SelectionDAG &DAG) const {
  ...
  if (ResTy.isInteger()) {
    ...
    EVT EltTy = VecTy.getVectorElementType();
    return DAG.getNode(RISCV64ISD::VEXTRACT_SEXT_ELT, DL, ResTy, Op0, Op1,
                       DAG.getValueType(EltTy));
  }
...
SDValue RISCV64SETargetLowering::lowerBUILD_VECTOR(SDValue Op,
                                                SelectionDAG &DAG) const {
...
static SDValue lowerVECTOR_SHUFFLE_SHF(SDValue Op, EVT ResTy,
  ...
  SDLoc DL(Op);
  return DAG.getNode(RISCV64ISD::SHF, DL, ResTy,
...
static SDValue lowerVECTOR_SHUFFLE_ILVEV(SDValue Op, EVT ResTy,
  ...
  return DAG.getNode(RISCV64ISD::ILVEV, SDLoc(Op), ResTy, Ws, Wt);
}
...
static SDValue lowerVECTOR_SHUFFLE_ILVOD(SDValue Op, EVT ResTy,
  ...
  return DAG.getNode(RISCV64ISD::ILVOD, SDLoc(Op), ResTy, Wt, Ws);
}
...
static SDValue lowerVECTOR_SHUFFLE_ILVR(SDValue Op, EVT ResTy,
  ...
  return DAG.getNode(RISCV64ISD::ILVR, SDLoc(Op), ResTy, Ws, Wt);
}
...
static SDValue lowerVECTOR_SHUFFLE_ILVL(SDValue Op, EVT ResTy,
  ...
  return DAG.getNode(RISCV64ISD::ILVL, SDLoc(Op), ResTy, Ws, Wt);
}
...
static SDValue lowerVECTOR_SHUFFLE_PCKEV(SDValue Op, EVT ResTy,
  ...
  return DAG.getNode(RISCV64ISD::PCKEV, SDLoc(Op), ResTy, Ws, Wt);
}
...
static SDValue lowerVECTOR_SHUFFLE_PCKOD(SDValue Op, EVT ResTy,
  ...
  return DAG.getNode(RISCV64ISD::PCKOD, SDLoc(Op), ResTy, Ws, Wt);
}
...
static SDValue lowerVECTOR_SHUFFLE_VSHF(SDValue Op, EVT ResTy,
  ...
  return DAG.getNode(RISCV64ISD::VSHF, DL, ResTy, MaskVec, Op1, Op0);
}

// Lower VECTOR_SHUFFLE into one of a number of instructions depending on the
// indices in the shuffle.
SDValue RISCV64SETargetLowering::lowerVECTOR_SHUFFLE(SDValue Op,
                                                  SelectionDAG &DAG) const {
...
MachineBasicBlock *
RISCV64SETargetLowering::emitBPOSGE32(MachineInstr &MI,
                                   MachineBasicBlock *BB) const {
  // $bb:
  //  bposge32_pseudo $vr0
  //  =>
  ...
  BuildMI(BB, DL, TII->get(RISCV64::BPOSGE32)).addMBB(TBB);
  // Insert the real bposge32c instruction to $BB.
  BuildMI(BB, DL, TII->get(RISCV64::BPOSGE32C_MMR3)).addMBB(TBB);

  // Fill $FBB.
  unsigned VR2 = RegInfo.createVirtualRegister(RC);
  BuildMI(*FBB, FBB->end(), DL, TII->get(RISCV64::ADDiu), VR2)
    .addReg(RISCV64::ZERO).addImm(0);
  BuildMI(*FBB, FBB->end(), DL, TII->get(RISCV64::B)).addMBB(Sink);

  // Fill $TBB.
  unsigned VR1 = RegInfo.createVirtualRegister(RC);
  BuildMI(*TBB, TBB->end(), DL, TII->get(RISCV64::ADDiu), VR1)
    .addReg(RISCV64::ZERO).addImm(1);

  // Insert phi function to $Sink.
  BuildMI(*Sink, Sink->begin(), DL, TII->get(RISCV64::PHI),
...
MachineBasicBlock *RISCV64SETargetLowering::emitMSACBranchPseudo(
  ...
  BuildMI(*FBB, FBB->end(), DL, TII->get(RISCV64::ADDiu), RD1)
    .addReg(RISCV64::ZERO).addImm(0);
  BuildMI(*FBB, FBB->end(), DL, TII->get(RISCV64::B)).addMBB(Sink);

  // Fill $TBB.
  unsigned RD2 = RegInfo.createVirtualRegister(RC);
  BuildMI(*TBB, TBB->end(), DL, TII->get(RISCV64::ADDiu), RD2)
    .addReg(RISCV64::ZERO).addImm(1);
...
MachineBasicBlock *
RISCV64SETargetLowering::emitCOPY_FW(MachineInstr &MI,
                                  MachineBasicBlock *BB) const {
  const TargetInstrInfo *TII = Subtarget.getInstrInfo();
  MachineRegisterInfo &RegInfo = BB->getParent()->getRegInfo();
  DebugLoc DL = MI.getDebugLoc();
  ...
  if (Lane == 0) {
    ...
    if (!Subtarget.useOddSPReg()) {
      ...
      Wt = RegInfo.createVirtualRegister(&RISCV64::MSA128WEvensRegClass);

      BuildMI(*BB, MI, DL, TII->get(RISCV64::COPY), Wt).addReg(Ws);
    }

    BuildMI(*BB, MI, DL, TII->get(RISCV64::COPY), Fd).addReg(Wt, 0, RISCV64::sub_lo);
  } else {
    unsigned Wt = RegInfo.createVirtualRegister(
        Subtarget.useOddSPReg() ? &RISCV64::MSA128WRegClass :
                                  &RISCV64::MSA128WEvensRegClass);

    BuildMI(*BB, MI, DL, TII->get(RISCV64::SPLATI_W), Wt).addReg(Ws).addImm(Lane);
    BuildMI(*BB, MI, DL, TII->get(RISCV64::COPY), Fd).addReg(Wt, 0, RISCV64::sub_lo);
  }
...
MachineBasicBlock *
RISCV64SETargetLowering::emitCOPY_FD(MachineInstr &MI,
                                  MachineBasicBlock *BB) const {
  ...
  if (Lane == 0)
    BuildMI(*BB, MI, DL, TII->get(RISCV64::COPY), Fd).addReg(Ws, 0, RISCV64::sub_64);
  else {
    unsigned Wt = RegInfo.createVirtualRegister(&RISCV64::MSA128DRegClass);

    BuildMI(*BB, MI, DL, TII->get(RISCV64::SPLATI_D), Wt).addReg(Ws).addImm(1);
    BuildMI(*BB, MI, DL, TII->get(RISCV64::COPY), Fd).addReg(Wt, 0, RISCV64::sub_64);
  }
...
MachineBasicBlock *
RISCV64SETargetLowering::emitINSERT_FW(MachineInstr &MI,
                                    MachineBasicBlock *BB) const {
  const TargetInstrInfo *TII = Subtarget.getInstrInfo();
  MachineRegisterInfo &RegInfo = BB->getParent()->getRegInfo();
  DebugLoc DL = MI.getDebugLoc();
  ...
  BuildMI(*BB, MI, DL, TII->get(RISCV64::SUBREG_TO_REG), Wt)
      .addImm(0)
      .addReg(Fs)
      .addImm(RISCV64::sub_lo);
  BuildMI(*BB, MI, DL, TII->get(RISCV64::INSVE_W), Wd)
      .addReg(Wd_in)
      .addImm(Lane)
      .addReg(Wt)
      .addImm(0);
...
MachineBasicBlock *
RISCV64SETargetLowering::emitINSERT_FD(MachineInstr &MI,
                                    MachineBasicBlock *BB) const {
  assert(Subtarget.isFP64bit());
  ...
  BuildMI(*BB, MI, DL, TII->get(RISCV64::SUBREG_TO_REG), Wt)
      .addImm(0)
      .addReg(Fs)
      .addImm(RISCV64::sub_64);
  BuildMI(*BB, MI, DL, TII->get(RISCV64::INSVE_D), Wd)
      .addReg(Wd_in)
      .addImm(Lane)
      .addReg(Wt)
      .addImm(0);
...
// (NEG $lanetmp2, $lanetmp1)
// (SLD_B $wd, $wdtmp2, $wdtmp2,  $lanetmp2)
MachineBasicBlock *RISCV64SETargetLowering::emitINSERT_DF_VIDX(
  ...
  const TargetRegisterClass *GPRRC =
      Subtarget.isABI_N64() ? &RISCV64::GPR64RegClass : &RISCV64::GPR32RegClass;
  unsigned SubRegIdx = Subtarget.isABI_N64() ? RISCV64::sub_32 : 0;
  unsigned ShiftOp = Subtarget.isABI_N64() ? RISCV64::DSLL : RISCV64::SLL;
  ...
  case 1:
    EltLog2Size = 0;
    InsertOp = RISCV64::INSERT_B;
    InsveOp = RISCV64::INSVE_B;
    VecRC = &RISCV64::MSA128BRegClass;
    break;
  case 2:
    EltLog2Size = 1;
    InsertOp = RISCV64::INSERT_H;
    InsveOp = RISCV64::INSVE_H;
    VecRC = &RISCV64::MSA128HRegClass;
    break;
  case 4:
    EltLog2Size = 2;
    InsertOp = RISCV64::INSERT_W;
    InsveOp = RISCV64::INSVE_W;
    VecRC = &RISCV64::MSA128WRegClass;
    break;
  case 8:
    EltLog2Size = 3;
    InsertOp = RISCV64::INSERT_D;
    InsveOp = RISCV64::INSVE_D;
    VecRC = &RISCV64::MSA128DRegClass;
    break;
  }
  ...
  if (IsFP) {
    ...
    BuildMI(*BB, MI, DL, TII->get(RISCV64::SUBREG_TO_REG), Wt)
        .addImm(0)
        .addReg(SrcValReg)
        .addImm(EltSizeInBytes == 8 ? RISCV64::sub_64 : RISCV64::sub_lo);
    SrcValReg = Wt;
  }
  ...
  BuildMI(*BB, MI, DL, TII->get(RISCV64::SLD_B), WdTmp1)
      .addReg(SrcVecReg)
      .addReg(SrcVecReg)
  ...
  BuildMI(*BB, MI, DL, TII->get(Subtarget.isABI_N64() ? RISCV64::DSUB : RISCV64::SUB),
          LaneTmp2)
      .addReg(Subtarget.isABI_N64() ? RISCV64::ZERO_64 : RISCV64::ZERO)
      .addReg(LaneReg);
  BuildMI(*BB, MI, DL, TII->get(RISCV64::SLD_B), Wd)
      .addReg(WdTmp2)
      .addReg(WdTmp2)
...
// splati.w $wd, $wt2[0]
MachineBasicBlock *
RISCV64SETargetLowering::emitFILL_FW(MachineInstr &MI,
                                  MachineBasicBlock *BB) const {
  const TargetInstrInfo *TII = Subtarget.getInstrInfo();
  MachineRegisterInfo &RegInfo = BB->getParent()->getRegInfo();
  DebugLoc DL = MI.getDebugLoc();
  ...
  BuildMI(*BB, MI, DL, TII->get(RISCV64::IMPLICIT_DEF), Wt1);
  BuildMI(*BB, MI, DL, TII->get(RISCV64::INSERT_SUBREG), Wt2)
      .addReg(Wt1)
      .addReg(Fs)
      .addImm(RISCV64::sub_lo);
  BuildMI(*BB, MI, DL, TII->get(RISCV64::SPLATI_W), Wd).addReg(Wt2).addImm(0);
...
// splati.d $wd, $wt2[0]
MachineBasicBlock *
RISCV64SETargetLowering::emitFILL_FD(MachineInstr &MI,
                                  MachineBasicBlock *BB) const {
  assert(Subtarget.isFP64bit());
  ...
  unsigned Wd = MI.getOperand(0).getReg();
  unsigned Fs = MI.getOperand(1).getReg();
  unsigned Wt1 = RegInfo.createVirtualRegister(&RISCV64::MSA128DRegClass);
  unsigned Wt2 = RegInfo.createVirtualRegister(&RISCV64::MSA128DRegClass);
  ...
  BuildMI(*BB, MI, DL, TII->get(RISCV64::INSERT_SUBREG), Wt2)
      ...
      .addReg(Fs)
      .addImm(RISCV64::sub_64);
  BuildMI(*BB, MI, DL, TII->get(RISCV64::SPLATI_D), Wd).addReg(Wt2).addImm(0);
...
// space.
MachineBasicBlock *
RISCV64SETargetLowering::emitST_F16_PSEUDO(MachineInstr &MI,
                                       MachineBasicBlock *BB) const {
  ...
  const TargetRegisterClass *RC =
      MI.getOperand(1).isReg() ? RegInfo.getRegClass(MI.getOperand(1).getReg())
                               : (Subtarget.isABI_O32() ? &RISCV64::GPR32RegClass
                                                        : &RISCV64::GPR64RegClass);
  const bool UsingRISCV6432 = RC == &RISCV64::GPR32RegClass;
  unsigned Rs = RegInfo.createVirtualRegister(&RISCV64::GPR32RegClass);

  BuildMI(*BB, MI, DL, TII->get(RISCV64::COPY_U_H), Rs).addReg(Ws).addImm(0);
  if(!UsingRISCV6432) {
    unsigned Tmp = RegInfo.createVirtualRegister(&RISCV64::GPR64RegClass);
    BuildMI(*BB, MI, DL, TII->get(RISCV64::SUBREG_TO_REG), Tmp)
        .addImm(0)
        .addReg(Rs)
        .addImm(RISCV64::sub_32);
    Rs = Tmp;
  }
  BuildMI(*BB, MI, DL, TII->get(UsingRISCV6432 ? RISCV64::SH : RISCV64::SH64))
      .addReg(Rs)
      .addReg(Rt)
      .addImm(Imm)
...
//     intervention.
MachineBasicBlock *
RISCV64SETargetLowering::emitLD_F16_PSEUDO(MachineInstr &MI,
                                       MachineBasicBlock *BB) const {
  ...
  MachineInstrBuilder MIB =
      BuildMI(*BB, MI, DL, TII->get(UsingRISCV6432 ? RISCV64::LH : RISCV64::LH64), Rt);
  for (unsigned i = 1; i < MI.getNumOperands(); i++)
    MIB.add(MI.getOperand(i));

  if(!UsingRISCV6432) {
    unsigned Tmp = RegInfo.createVirtualRegister(&RISCV64::GPR32RegClass);
    BuildMI(*BB, MI, DL, TII->get(RISCV64::COPY), Tmp).addReg(Rt, 0, RISCV64::sub_32);
    Rt = Tmp;
  }

  BuildMI(*BB, MI, DL, TII->get(RISCV64::FILL_H), Wd).addReg(Rt);
...
MachineBasicBlock *
RISCV64SETargetLowering::emitFPROUND_PSEUDO(MachineInstr &MI,
  ...
  // it.
  assert(Subtarget.hasMSA() && Subtarget.hasRISCV6432r2());

  bool IsFGR64onRISCV6464 = Subtarget.hasRISCV6464() && IsFGR64;
  bool IsFGR64onRISCV6432 = !Subtarget.hasRISCV6464() && IsFGR64;
  ...
  MachineRegisterInfo &RegInfo = BB->getParent()->getRegInfo();
  unsigned Wtemp = RegInfo.createVirtualRegister(&RISCV64::MSA128WRegClass);
  const TargetRegisterClass *GPRRC =
      IsFGR64onRISCV6464 ? &RISCV64::GPR64RegClass : &RISCV64::GPR32RegClass;
  unsigned MFC1Opc = IsFGR64onRISCV6464
                         ? RISCV64::DMFC1
                         : (IsFGR64onRISCV6432 ? RISCV64::MFC1_D64 : RISCV64::MFC1);
  unsigned FILLOpc = IsFGR64onRISCV6464 ? RISCV64::FILL_D : RISCV64::FILL_W;
  ...
  if (IsFGR64onRISCV6432) {
    unsigned Rtemp2 = RegInfo.createVirtualRegister(GPRRC);
    BuildMI(*BB, MI, DL, TII->get(RISCV64::MFHC1_D64), Rtemp2).addReg(Fs);
    unsigned Wtemp2 = RegInfo.createVirtualRegister(&RISCV64::MSA128WRegClass);
    unsigned Wtemp3 = RegInfo.createVirtualRegister(&RISCV64::MSA128WRegClass);
    BuildMI(*BB, MI, DL, TII->get(RISCV64::INSERT_W), Wtemp2)
        .addReg(Wtemp)
        .addReg(Rtemp2)
        .addImm(1);
    BuildMI(*BB, MI, DL, TII->get(RISCV64::INSERT_W), Wtemp3)
        .addReg(Wtemp2)
        .addReg(Rtemp2)
        .addImm(3);
    WPHI = Wtemp3;
  ...
  if (IsFGR64) {
    unsigned Wtemp2 = RegInfo.createVirtualRegister(&RISCV64::MSA128WRegClass);
    BuildMI(*BB, MI, DL, TII->get(RISCV64::FEXDO_W), Wtemp2)
        .addReg(WPHI)
        .addReg(WPHI);
    WPHI = Wtemp2;
  }

  BuildMI(*BB, MI, DL, TII->get(RISCV64::FEXDO_H), Wd).addReg(WPHI).addReg(WPHI);
...
MachineBasicBlock *
RISCV64SETargetLowering::emitFPEXTEND_PSEUDO(MachineInstr &MI,
                                          MachineBasicBlock *BB,
                                          bool IsFGR64) const {
  ...
  const TargetRegisterClass *GPRRC =
      IsFGR64onRISCV6464 ? &RISCV64::GPR64RegClass : &RISCV64::GPR32RegClass;
  unsigned MTC1Opc = IsFGR64onRISCV6464
                         ? RISCV64::DMTC1
                         : (IsFGR64onRISCV6432 ? RISCV64::MTC1_D64 : RISCV64::MTC1);
  unsigned COPYOpc = IsFGR64onRISCV6464 ? RISCV64::COPY_S_D : RISCV64::COPY_S_W;

  unsigned Wtemp = RegInfo.createVirtualRegister(&RISCV64::MSA128WRegClass);
  unsigned WPHI = Wtemp;

  BuildMI(*BB, MI, DL, TII->get(RISCV64::FEXUPR_W), Wtemp).addReg(Ws);
  if (IsFGR64) {
    WPHI = RegInfo.createVirtualRegister(&RISCV64::MSA128DRegClass);
    BuildMI(*BB, MI, DL, TII->get(RISCV64::FEXUPR_D), WPHI).addReg(Wtemp);
  }

  // Perform the safety regclass copy mentioned above.
  unsigned Rtemp = RegInfo.createVirtualRegister(GPRRC);
  unsigned FPRPHI = IsFGR64onRISCV6432
                        ? RegInfo.createVirtualRegister(&RISCV64::FGR64RegClass)
                        : Fd;
  ...
  if (IsFGR64onRISCV6432) {
    unsigned Rtemp2 = RegInfo.createVirtualRegister(GPRRC);
    BuildMI(*BB, MI, DL, TII->get(RISCV64::COPY_S_W), Rtemp2)
        .addReg(WPHI)
        .addImm(1);
    BuildMI(*BB, MI, DL, TII->get(RISCV64::MTHC1_D64), Fd)
        .addReg(FPRPHI)
        .addReg(Rtemp2);
  }
...
// fexp2.w $wd, $ws, $wt
MachineBasicBlock *
RISCV64SETargetLowering::emitFEXP2_W_1(MachineInstr &MI,
                                    MachineBasicBlock *BB) const {
  const TargetInstrInfo *TII = Subtarget.getInstrInfo();
  MachineRegisterInfo &RegInfo = BB->getParent()->getRegInfo();
  const TargetRegisterClass *RC = &RISCV64::MSA128WRegClass;
  unsigned Ws1 = RegInfo.createVirtualRegister(RC);
  unsigned Ws2 = RegInfo.createVirtualRegister(RC);
  DebugLoc DL = MI.getDebugLoc();

  // Splat 1.0 into a vector
  BuildMI(*BB, MI, DL, TII->get(RISCV64::LDI_W), Ws1).addImm(1);
  BuildMI(*BB, MI, DL, TII->get(RISCV64::FFINT_U_W), Ws2).addReg(Ws1);

  // Emit 1.0 * fexp2(Wt)
  BuildMI(*BB, MI, DL, TII->get(RISCV64::FEXP2_W), MI.getOperand(0).getReg())
      .addReg(Ws2)
      .addReg(MI.getOperand(1).getReg());
...
// fexp2.d $wd, $ws, $wt
MachineBasicBlock *
RISCV64SETargetLowering::emitFEXP2_D_1(MachineInstr &MI,
                                    MachineBasicBlock *BB) const {
  const TargetInstrInfo *TII = Subtarget.getInstrInfo();
  MachineRegisterInfo &RegInfo = BB->getParent()->getRegInfo();
  const TargetRegisterClass *RC = &RISCV64::MSA128DRegClass;
  unsigned Ws1 = RegInfo.createVirtualRegister(RC);
  unsigned Ws2 = RegInfo.createVirtualRegister(RC);
  DebugLoc DL = MI.getDebugLoc();

  // Splat 1.0 into a vector
  BuildMI(*BB, MI, DL, TII->get(RISCV64::LDI_D), Ws1).addImm(1);
  BuildMI(*BB, MI, DL, TII->get(RISCV64::FFINT_U_D), Ws2).addReg(Ws1);

  // Emit 1.0 * fexp2(Wt)
  BuildMI(*BB, MI, DL, TII->get(RISCV64::FEXP2_D), MI.getOperand(0).getReg())
      .addReg(Ws2)
      .addReg(MI.getOperand(1).getReg());
