...
class RISCV64BranchExpansion : public MachineFunctionPass {
public:
  static char ID;

  RISCV64BranchExpansion() : MachineFunctionPass(ID), ABI(RISCV64ABIInfo::Unknown()) {
    initializeRISCV64BranchExpansionPass(*PassRegistry::getPassRegistry());
  }

  StringRef getPassName() const override {
    return "RISCV64 Branch Expansion Pass";
  }
...
char RISCV64BranchExpansion::ID = 0;

INITIALIZE_PASS(RISCV64BranchExpansion, DEBUG_TYPE,
                "Expand out of range branch instructions and fix forbidden"
                " slot hazards",
                false, false)

/// Returns a pass that clears pipeline hazards.
FunctionPass *llvm::createRISCV64BranchExpansion() {
  return new RISCV64BranchExpansion();
}
...
void RISCV64BranchExpansion::splitMBB(MachineBasicBlock *MBB) {
...
// Fill MBBInfos.
void RISCV64BranchExpansion::initMBBInfo() {
...
int64_t RISCV64BranchExpansion::computeOffset(const MachineInstr *Br) {
  int64_t Offset = 0;
...
uint64_t RISCV64BranchExpansion::computeOffsetFromTheBeginning(int MBB) {
  uint64_t Offset = 0;
  for (int N = 0; N < MBB; ++N)
    Offset += MBBInfos[N].Size;
  return Offset;
...
void RISCV64BranchExpansion::replaceBranch(MachineBasicBlock &MBB, Iter Br,
                                        const DebugLoc &DL,
                                        MachineBasicBlock *MBBOpnd) {
...
bool RISCV64BranchExpansion::buildProperJumpMI(MachineBasicBlock *MBB,
                                            MachineBasicBlock::iterator Pos,
                                            DebugLoc DL) {
  bool HasR6 = ABI.IsN64() ? STI->hasRISCV6464r6() : STI->hasRISCV6432r6();
  bool AddImm = HasR6 && !STI->useIndirectJumpsHazard();

  unsigned JR = ABI.IsN64() ? RISCV64::JR64 : RISCV64::JR;
  unsigned JIC = ABI.IsN64() ? RISCV64::JIC64 : RISCV64::JIC;
  unsigned JR_HB = ABI.IsN64() ? RISCV64::JR_HB64 : RISCV64::JR_HB;
  unsigned JR_HB_R6 = ABI.IsN64() ? RISCV64::JR_HB64_R6 : RISCV64::JR_HB_R6;
  ...
  if (JumpOp == RISCV64::JIC && STI->inMicroRISCV64Mode())
    JumpOp = RISCV64::JIC_MMR6;

  unsigned ATReg = ABI.IsN64() ? RISCV64::AT_64 : RISCV64::AT;
...
// are present.
void RISCV64BranchExpansion::expandToLongBranch(MBBInfo &I) {
  MachineBasicBlock::iterator Pos;
  ...
  if (IsPIC) {
    ...
    const unsigned BalOp =
        STI->hasRISCV6432r6()
            ? STI->inMicroRISCV64Mode() ? RISCV64::BALC_MMR6 : RISCV64::BALC
            : STI->inMicroRISCV64Mode() ? RISCV64::BAL_BR_MM : RISCV64::BAL_BR;
    ...
    if (!ABI.IsN64()) {
      ...
      BuildMI(*LongBrMBB, Pos, DL, TII->get(RISCV64::ADDiu), RISCV64::SP)
          .addReg(RISCV64::SP)
          .addImm(-8);
      BuildMI(*LongBrMBB, Pos, DL, TII->get(RISCV64::SW))
          .addReg(RISCV64::RA)
          .addReg(RISCV64::SP)
          .addImm(0);
      ...
      BuildMI(*LongBrMBB, Pos, DL, TII->get(RISCV64::LONG_BRANCH_LUi), RISCV64::AT)
          .addMBB(TgtMBB, RISCV64II::MO_ABS_HI)
          .addMBB(BalTgtMBB);

      MachineInstrBuilder BalInstr =
          BuildMI(*MFp, DL, TII->get(BalOp)).addMBB(BalTgtMBB);
      MachineInstrBuilder ADDiuInstr =
          BuildMI(*MFp, DL, TII->get(RISCV64::LONG_BRANCH_ADDiu), RISCV64::AT)
              .addReg(RISCV64::AT)
              .addMBB(TgtMBB, RISCV64II::MO_ABS_LO)
              .addMBB(BalTgtMBB);
      if (STI->hasRISCV6432r6()) {
      ...
      BuildMI(*BalTgtMBB, Pos, DL, TII->get(RISCV64::ADDu), RISCV64::AT)
          .addReg(RISCV64::RA)
          .addReg(RISCV64::AT);
      BuildMI(*BalTgtMBB, Pos, DL, TII->get(RISCV64::LW), RISCV64::RA)
          .addReg(RISCV64::SP)
          .addImm(0);
      if (STI->isTargetNaCl())
      ...
      if (STI->isTargetNaCl() || !hasDelaySlot) {
        BuildMI(*BalTgtMBB, std::prev(Pos), DL, TII->get(RISCV64::ADDiu), RISCV64::SP)
            .addReg(RISCV64::SP)
            .addImm(8);
      }
      if (hasDelaySlot) {
        if (STI->isTargetNaCl()) {
          BuildMI(*BalTgtMBB, Pos, DL, TII->get(RISCV64::NOP));
        } else {
          BuildMI(*BalTgtMBB, Pos, DL, TII->get(RISCV64::ADDiu), RISCV64::SP)
              .addReg(RISCV64::SP)
              .addImm(8);
        }
      ...
      BuildMI(*LongBrMBB, Pos, DL, TII->get(RISCV64::DADDiu), RISCV64::SP_64)
          .addReg(RISCV64::SP_64)
          .addImm(-16);
      BuildMI(*LongBrMBB, Pos, DL, TII->get(RISCV64::SD))
          .addReg(RISCV64::RA_64)
          .addReg(RISCV64::SP_64)
          .addImm(0);
      BuildMI(*LongBrMBB, Pos, DL, TII->get(RISCV64::LONG_BRANCH_DADDiu),
              RISCV64::AT_64)
          .addReg(RISCV64::ZERO_64)
          .addMBB(TgtMBB, RISCV64II::MO_ABS_HI)
          .addMBB(BalTgtMBB);
      BuildMI(*LongBrMBB, Pos, DL, TII->get(RISCV64::DSLL), RISCV64::AT_64)
          .addReg(RISCV64::AT_64)
          .addImm(16);

      MachineInstrBuilder BalInstr =
          BuildMI(*MFp, DL, TII->get(BalOp)).addMBB(BalTgtMBB);
      MachineInstrBuilder DADDiuInstr =
          BuildMI(*MFp, DL, TII->get(RISCV64::LONG_BRANCH_DADDiu), RISCV64::AT_64)
              .addReg(RISCV64::AT_64)
              .addMBB(TgtMBB, RISCV64II::MO_ABS_LO)
              .addMBB(BalTgtMBB);
      if (STI->hasRISCV6432r6()) {
      ...
      BuildMI(*BalTgtMBB, Pos, DL, TII->get(RISCV64::DADDu), RISCV64::AT_64)
          .addReg(RISCV64::RA_64)
          .addReg(RISCV64::AT_64);
      BuildMI(*BalTgtMBB, Pos, DL, TII->get(RISCV64::LD), RISCV64::RA_64)
          .addReg(RISCV64::SP_64)
          .addImm(0);
      ...
      if (!hasDelaySlot) {
        BuildMI(*BalTgtMBB, std::prev(Pos), DL, TII->get(RISCV64::DADDiu),
                RISCV64::SP_64)
            .addReg(RISCV64::SP_64)
            .addImm(16);
      } else {
        BuildMI(*BalTgtMBB, Pos, DL, TII->get(RISCV64::DADDiu), RISCV64::SP_64)
            .addReg(RISCV64::SP_64)
            .addImm(16);
        BalTgtMBB->rbegin()->bundleWithPred();
      }
    ...
    if (STI->hasRISCV6432r6() && TII->isBranchOffsetInRange(RISCV64::BC, I.Offset)) {
      // R6:
      // $longbr:
      //  bc $tgt
      // $fallthrough:
      ...
      BuildMI(*LongBrMBB, Pos, DL,
              TII->get(STI->inMicroRISCV64Mode() ? RISCV64::BC_MMR6 : RISCV64::BC))
          .addMBB(TgtMBB);
      ...
      MIBundleBuilder(*LongBrMBB, Pos)
          .append(BuildMI(*MFp, DL, TII->get(RISCV64::J)).addMBB(TgtMBB))
          .append(BuildMI(*MFp, DL, TII->get(RISCV64::NOP)));
    } else {
      ...
      // do branch register.
      if (ABI.IsN64()) {
        BuildMI(*LongBrMBB, Pos, DL, TII->get(RISCV64::LONG_BRANCH_LUi2Op_64),
                RISCV64::AT_64)
            .addMBB(TgtMBB, RISCV64II::MO_HIGHEST);
        BuildMI(*LongBrMBB, Pos, DL, TII->get(RISCV64::LONG_BRANCH_DADDiu2Op),
                RISCV64::AT_64)
            .addReg(RISCV64::AT_64)
            .addMBB(TgtMBB, RISCV64II::MO_HIGHER);
        BuildMI(*LongBrMBB, Pos, DL, TII->get(RISCV64::DSLL), RISCV64::AT_64)
            .addReg(RISCV64::AT_64)
            .addImm(16);
        BuildMI(*LongBrMBB, Pos, DL, TII->get(RISCV64::LONG_BRANCH_DADDiu2Op),
        ...
        BuildMI(*LongBrMBB, Pos, DL, TII->get(RISCV64::LONG_BRANCH_LUi2Op),
                RISCV64::AT)
            .addMBB(TgtMBB, RISCV64II::MO_ABS_HI);
        BuildMI(*LongBrMBB, Pos, DL, TII->get(RISCV64::LONG_BRANCH_ADDiu2Op),
                RISCV64::AT)
            .addReg(RISCV64::AT)
            .addMBB(TgtMBB, RISCV64II::MO_ABS_LO);
      }
...
static void emitGPDisp(MachineFunction &F, const RISCV64InstrInfo *TII) {
  MachineBasicBlock &MBB = F.front();
  MachineBasicBlock::iterator I = MBB.begin();
  DebugLoc DL = MBB.findDebugLoc(MBB.begin());
  BuildMI(MBB, I, DL, TII->get(RISCV64::LUi), RISCV64::V0)
      .addExternalSymbol("_gp_disp", RISCV64II::MO_ABS_HI);
  BuildMI(MBB, I, DL, TII->get(RISCV64::ADDiu), RISCV64::V0)
      .addReg(RISCV64::V0)
      .addExternalSymbol("_gp_disp", RISCV64II::MO_ABS_LO);
  MBB.removeLiveIn(RISCV64::V0);
}

bool RISCV64BranchExpansion::handleForbiddenSlot() {
  // Forbidden slot hazards are only defined for MIPSR6 but not microMIPSR6.
  if (!STI->hasRISCV6432r6() || STI->inMicroRISCV64Mode())
    return false;
    ...
    for (Iter I = FI->begin(); I != FI->end(); ++I) {
      ...
      if (LastInstInFunction || !TII->SafeInForbiddenSlot(*Inst)) {
        ...
        if (std::next(Iit) == FI->end() ||
            std::next(Iit)->getOpcode() != RISCV64::NOP) {
          Changed = true;
          MIBundleBuilder(&*I).append(
              BuildMI(*MFp, I->getDebugLoc(), TII->get(RISCV64::NOP)));
          NumInsertedNops++;
        }
      }
    }
...
bool RISCV64BranchExpansion::handlePossibleLongBranch() {
  if (STI->inRISCV6416Mode() || !STI->enableLongBranchPass())
    return false;
...
bool RISCV64BranchExpansion::runOnMachineFunction(MachineFunction &MF) {
  const TargetMachine &TM = MF.getTarget();
  IsPIC = TM.isPositionIndependent();
  ABI = static_cast<const RISCV64TargetMachine &>(TM).getABI();
  STI = &static_cast<const RISCV64Subtarget &>(MF.getSubtarget());
  TII = static_cast<const RISCV64InstrInfo *>(STI->getInstrInfo());

  if (IsPIC && ABI.IsO32() &&
      MF.getInfo<RISCV64FunctionInfo>()->globalBaseRegSet())
    emitGPDisp(MF, TII);
