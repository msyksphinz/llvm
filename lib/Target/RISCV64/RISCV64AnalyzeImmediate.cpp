//===- RISCV64AnalyzeImmediate.cpp - Analyze Immediates ----------------------===//
//
...
RISCV64AnalyzeImmediate::Inst::Inst(unsigned O, unsigned I) : Opc(O), ImmOpnd(I) {}

// Add I to the instruction sequences.
void RISCV64AnalyzeImmediate::AddInstr(InstSeqLs &SeqLs, const Inst &I) {
...
void RISCV64AnalyzeImmediate::GetInstSeqLsADDiu(uint64_t Imm, unsigned RemSize,
                                             InstSeqLs &SeqLs) {
  GetInstSeqLs((Imm + 0x8000ULL) & 0xffffffffffff0000ULL, RemSize, SeqLs);
  AddInstr(SeqLs, Inst(ADDiu, Imm & 0xffffULL));
}

void RISCV64AnalyzeImmediate::GetInstSeqLsORi(uint64_t Imm, unsigned RemSize,
                                           InstSeqLs &SeqLs) {
  GetInstSeqLs(Imm & 0xffffffffffff0000ULL, RemSize, SeqLs);
  AddInstr(SeqLs, Inst(ORi, Imm & 0xffffULL));
}

void RISCV64AnalyzeImmediate::GetInstSeqLsSLL(uint64_t Imm, unsigned RemSize,
                                           InstSeqLs &SeqLs) {
...
void RISCV64AnalyzeImmediate::GetInstSeqLs(uint64_t Imm, unsigned RemSize,
                                        InstSeqLs &SeqLs) {
...
// are replaced with
//  LUi 0x444
void RISCV64AnalyzeImmediate::ReplaceADDiuSLLWithLUi(InstSeq &Seq) {
...
void RISCV64AnalyzeImmediate::GetShortestSeq(InstSeqLs &SeqLs, InstSeq &Insts) {
...
const RISCV64AnalyzeImmediate::InstSeq
&RISCV64AnalyzeImmediate::Analyze(uint64_t Imm, unsigned Size,
