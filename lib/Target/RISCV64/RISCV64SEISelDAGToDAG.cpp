//===-- RISCV64SEISelDAGToDAG.cpp - A Dag to Dag Inst Selector for RISCV64SE ----===//
//
...
//===----------------------------------------------------------------------===//
//
// Subclass of RISCV64DAGToDAGISel specialized for mips32/64.
//
//===----------------------------------------------------------------------===//
...
bool RISCV64SEDAGToDAGISel::runOnMachineFunction(MachineFunction &MF) {
...
void RISCV64SEDAGToDAGISel::getAnalysisUsage(AnalysisUsage &AU) const {
  AU.addRequired<DominatorTreeWrapperPass>();
  SelectionDAGISel::getAnalysisUsage(AU);
}

void RISCV64SEDAGToDAGISel::addDSPCtrlRegOperands(bool IsDef, MachineInstr &MI,
                                               MachineFunction &MF) {
  ...
  if (Mask & 1)
    MIB.addReg(RISCV64::DSPPos, Flag);

  if (Mask & 2)
    MIB.addReg(RISCV64::DSPSCount, Flag);

  if (Mask & 4)
    MIB.addReg(RISCV64::DSPCarry, Flag);

  if (Mask & 8)
    MIB.addReg(RISCV64::DSPOutFlag, Flag);

  if (Mask & 16)
    MIB.addReg(RISCV64::DSPCCond, Flag);

  if (Mask & 32)
    MIB.addReg(RISCV64::DSPEFI, Flag);
}

unsigned RISCV64SEDAGToDAGISel::getMSACtrlReg(const SDValue RegIdx) const {
  switch (cast<ConstantSDNode>(RegIdx)->getZExtValue()) {
  default:
    llvm_unreachable("Could not map int to register");
  case 0: return RISCV64::MSAIR;
  case 1: return RISCV64::MSACSR;
  case 2: return RISCV64::MSAAccess;
  case 3: return RISCV64::MSASave;
  case 4: return RISCV64::MSAModify;
  case 5: return RISCV64::MSARequest;
  case 6: return RISCV64::MSAMap;
  case 7: return RISCV64::MSAUnmap;
  }
}

bool RISCV64SEDAGToDAGISel::replaceUsesWithZeroReg(MachineRegisterInfo *MRI,
                                                const MachineInstr& MI) {
  unsigned DstReg = 0, ZeroReg = 0;

  // Check if MI is "addiu $dst, $zero, 0" or "daddiu $dst, $zero, 0".
  if ((MI.getOpcode() == RISCV64::ADDiu) &&
      (MI.getOperand(1).getReg() == RISCV64::ZERO) &&
      (MI.getOperand(2).isImm()) &&
      (MI.getOperand(2).getImm() == 0)) {
    DstReg = MI.getOperand(0).getReg();
    ZeroReg = RISCV64::ZERO;
  } else if ((MI.getOpcode() == RISCV64::DADDiu) &&
             (MI.getOperand(1).getReg() == RISCV64::ZERO_64) &&
             (MI.getOperand(2).isImm()) &&
             (MI.getOperand(2).getImm() == 0)) {
    DstReg = MI.getOperand(0).getReg();
...
void RISCV64SEDAGToDAGISel::initGlobalBaseReg(MachineFunction &MF) {
  ...
  DebugLoc DL;
  unsigned V0, V1, GlobalBaseReg = RISCV64FI->getGlobalBaseReg();
  const TargetRegisterClass *RC;
  const RISCV64ABIInfo &ABI = static_cast<const RISCV64TargetMachine &>(TM).getABI();
  RC = (ABI.IsN64()) ? &RISCV64::GPR64RegClass : &RISCV64::GPR32RegClass;

  V0 = RegInfo.createVirtualRegister(RC);
  V1 = RegInfo.createVirtualRegister(RC);

  if (ABI.IsN64()) {
    MF.getRegInfo().addLiveIn(RISCV64::T9_64);
    MBB.addLiveIn(RISCV64::T9_64);

    // lui $v0, %hi(%neg(%gp_rel(fname)))
    // daddu $v1, $v0, $t9
    // daddiu $globalbasereg, $v1, %lo(%neg(%gp_rel(fname)))
    const GlobalValue *FName = &MF.getFunction();
    BuildMI(MBB, I, DL, TII.get(RISCV64::LUi64), V0)
      .addGlobalAddress(FName, 0, RISCV64II::MO_GPOFF_HI);
    BuildMI(MBB, I, DL, TII.get(RISCV64::DADDu), V1).addReg(V0)
      .addReg(RISCV64::T9_64);
    BuildMI(MBB, I, DL, TII.get(RISCV64::DADDiu), GlobalBaseReg).addReg(V1)
      .addGlobalAddress(FName, 0, RISCV64II::MO_GPOFF_LO);
    return;
  }
  ...
  if (!MF.getTarget().isPositionIndependent()) {
    ...
    BuildMI(MBB, I, DL, TII.get(RISCV64::LUi), V0)
      .addExternalSymbol("__gnu_local_gp", RISCV64II::MO_ABS_HI);
    BuildMI(MBB, I, DL, TII.get(RISCV64::ADDiu), GlobalBaseReg).addReg(V0)
      .addExternalSymbol("__gnu_local_gp", RISCV64II::MO_ABS_LO);
    return;
  }

  MF.getRegInfo().addLiveIn(RISCV64::T9);
  MBB.addLiveIn(RISCV64::T9);
  ...
  if (ABI.IsN32()) {
    ...
    BuildMI(MBB, I, DL, TII.get(RISCV64::LUi), V0)
      .addGlobalAddress(FName, 0, RISCV64II::MO_GPOFF_HI);
    BuildMI(MBB, I, DL, TII.get(RISCV64::ADDu), V1).addReg(V0).addReg(RISCV64::T9);
    BuildMI(MBB, I, DL, TII.get(RISCV64::ADDiu), GlobalBaseReg).addReg(V1)
      .addGlobalAddress(FName, 0, RISCV64II::MO_GPOFF_LO);
    return;
  }
  ...
  //
  // Register $2 (RISCV64::V0) is added to the list of live-in registers to ensure
  // the value instruction 1 (addiu) defines is valid when instruction 2 (addu)
  // reads it.
  MF.getRegInfo().addLiveIn(RISCV64::V0);
  MBB.addLiveIn(RISCV64::V0);
  BuildMI(MBB, I, DL, TII.get(RISCV64::ADDu), GlobalBaseReg)
    .addReg(RISCV64::V0).addReg(RISCV64::T9);
}

void RISCV64SEDAGToDAGISel::processFunctionAfterISel(MachineFunction &MF) {
    ...
    for (auto &MI: MBB) {
      ...
      case RISCV64::ExtractElementF64_64:
        if (!Subtarget->useOddSPReg()) {
          MI.addOperand(MachineOperand::CreateReg(RISCV64::SP, false, true));
          break;
        }
        LLVM_FALLTHROUGH;
...
void RISCV64SEDAGToDAGISel::selectAddE(SDNode *Node, const SDLoc &DL) const {
  ...
  if (Opc == ISD::ADDC) {
    ...
    CurDAG->SelectNodeTo(Node, RISCV64::ADDWC, VT, MVT::Glue, Ops);
    return;
  }
  ...
  // carry field. The usage of 1 here with RISCV64ISD::RDDSP / RISCV64::WRDSP
  ...
  SDNode *DSPCtrlField =
      CurDAG->getMachineNode(RISCV64::RDDSP, DL, MVT::i32, MVT::Glue, CstOne, InFlag);

  SDNode *Carry = CurDAG->getMachineNode(
      RISCV64::EXT, DL, MVT::i32, SDValue(DSPCtrlField, 0), OuFlag, CstOne);

  SDValue Ops[4] = {SDValue(DSPCtrlField, 0),
                    CurDAG->getTargetConstant(6, DL, MVT::i32), CstOne,
                    SDValue(Carry, 0)};
  SDNode *DSPCFWithCarry = CurDAG->getMachineNode(RISCV64::INS, DL, MVT::i32, Ops);
  ...
  SDValue Zero = CurDAG->getRegister(RISCV64::ZERO, MVT::i32);

  SDValue InsOps[4] = {Zero, OuFlag, CstOne, SDValue(DSPCFWithCarry, 0)};
  SDNode *DSPCtrlFinal = CurDAG->getMachineNode(RISCV64::INS, DL, MVT::i32, InsOps);

  SDNode *WrDSP = CurDAG->getMachineNode(RISCV64::WRDSP, DL, MVT::Glue,
                                         SDValue(DSPCtrlFinal, 0), CstOne);

  SDValue Operands[3] = {LHS, RHS, SDValue(WrDSP, 0)};
  CurDAG->SelectNodeTo(Node, RISCV64::ADDWC, VT, MVT::Glue, Operands);
}

/// Match frameindex
bool RISCV64SEDAGToDAGISel::selectAddrFrameIndex(SDValue Addr, SDValue &Base,
...
bool RISCV64SEDAGToDAGISel::selectAddrFrameIndexOffset(
...
/// ComplexPattern used on RISCV64InstrInfo
/// Used on RISCV64 Load/Store instructions
bool RISCV64SEDAGToDAGISel::selectAddrRegImm(SDValue Addr, SDValue &Base,
                                          SDValue &Offset) const {
...
/// ComplexPattern used on RISCV64InstrInfo
/// Used on RISCV64 Load/Store instructions
bool RISCV64SEDAGToDAGISel::selectAddrDefault(SDValue Addr, SDValue &Base,
                                           SDValue &Offset) const {
  Base = Addr;
...
bool RISCV64SEDAGToDAGISel::selectIntAddr(SDValue Addr, SDValue &Base,
                                       SDValue &Offset) const {
  return selectAddrRegImm(Addr, Base, Offset) ||
    selectAddrDefault(Addr, Base, Offset);
}

bool RISCV64SEDAGToDAGISel::selectAddrRegImm9(SDValue Addr, SDValue &Base,
                                           SDValue &Offset) const {
  if (selectAddrFrameIndex(Addr, Base, Offset))
    return true;
...
bool RISCV64SEDAGToDAGISel::selectAddrRegImm11(SDValue Addr, SDValue &Base,
                                            SDValue &Offset) const {
  if (selectAddrFrameIndex(Addr, Base, Offset))
    return true;
...
bool RISCV64SEDAGToDAGISel::selectAddrRegImm12(SDValue Addr, SDValue &Base,
                                            SDValue &Offset) const {
  if (selectAddrFrameIndex(Addr, Base, Offset))
    return true;
...
bool RISCV64SEDAGToDAGISel::selectAddrRegImm16(SDValue Addr, SDValue &Base,
                                            SDValue &Offset) const {
  if (selectAddrFrameIndex(Addr, Base, Offset))
    return true;
...
bool RISCV64SEDAGToDAGISel::selectIntAddr11MM(SDValue Addr, SDValue &Base,
                                         SDValue &Offset) const {
  return selectAddrRegImm11(Addr, Base, Offset) ||
    selectAddrDefault(Addr, Base, Offset);
}

bool RISCV64SEDAGToDAGISel::selectIntAddr12MM(SDValue Addr, SDValue &Base,
                                         SDValue &Offset) const {
  return selectAddrRegImm12(Addr, Base, Offset) ||
    selectAddrDefault(Addr, Base, Offset);
}

bool RISCV64SEDAGToDAGISel::selectIntAddr16MM(SDValue Addr, SDValue &Base,
                                         SDValue &Offset) const {
  return selectAddrRegImm16(Addr, Base, Offset) ||
    selectAddrDefault(Addr, Base, Offset);
}

bool RISCV64SEDAGToDAGISel::selectIntAddrLSL2MM(SDValue Addr, SDValue &Base,
                                             SDValue &Offset) const {
...
bool RISCV64SEDAGToDAGISel::selectIntAddrSImm10(SDValue Addr, SDValue &Base,
                                             SDValue &Offset) const {
...
bool RISCV64SEDAGToDAGISel::selectIntAddrSImm10Lsl1(SDValue Addr, SDValue &Base,
                                                 SDValue &Offset) const {
  if (selectAddrFrameIndex(Addr, Base, Offset))
    return true;
...
bool RISCV64SEDAGToDAGISel::selectIntAddrSImm10Lsl2(SDValue Addr, SDValue &Base,
                                                 SDValue &Offset) const {
  if (selectAddrFrameIndex(Addr, Base, Offset))
    return true;
...
bool RISCV64SEDAGToDAGISel::selectIntAddrSImm10Lsl3(SDValue Addr, SDValue &Base,
                                                 SDValue &Offset) const {
  if (selectAddrFrameIndex(Addr, Base, Offset))
    return true;
...
bool RISCV64SEDAGToDAGISel::selectVSplat(SDNode *N, APInt &Imm,
...
// RISCV64SEDAGToDAGISel::selectNode.
bool RISCV64SEDAGToDAGISel::
...
// TODO: This might not be appropriate for big-endian MSA since BITCAST is
//       sometimes a shuffle in big-endian mode.
bool RISCV64SEDAGToDAGISel::selectVSplatUimmPow2(SDValue N, SDValue &Imm) const {
  APInt ImmValue;
  EVT EltTy = N->getValueType(0).getVectorElementType();
...
// TODO: This might not be appropriate for big-endian MSA since BITCAST is
//       sometimes a shuffle in big-endian mode.
bool RISCV64SEDAGToDAGISel::selectVSplatMaskL(SDValue N, SDValue &Imm) const {
  APInt ImmValue;
  EVT EltTy = N->getValueType(0).getVectorElementType();
...
// TODO: This might not be appropriate for big-endian MSA since BITCAST is
//       sometimes a shuffle in big-endian mode.
bool RISCV64SEDAGToDAGISel::selectVSplatMaskR(SDValue N, SDValue &Imm) const {
  APInt ImmValue;
  EVT EltTy = N->getValueType(0).getVectorElementType();
...
bool RISCV64SEDAGToDAGISel::selectVSplatUimmInvPow2(SDValue N,
                                                 SDValue &Imm) const {
  APInt ImmValue;
  EVT EltTy = N->getValueType(0).getVectorElementType();
...
bool RISCV64SEDAGToDAGISel::trySelect(SDNode *Node) {
    ...
    if (Node->getValueType(0) == MVT::f64 && CN->isExactlyValue(+0.0)) {
      if (Subtarget->isGP64bit()) {
        SDValue Zero = CurDAG->getCopyFromReg(CurDAG->getEntryNode(), DL,
                                              RISCV64::ZERO_64, MVT::i64);
        ReplaceNode(Node,
                    CurDAG->getMachineNode(RISCV64::DMTC1, DL, MVT::f64, Zero));
      } else if (Subtarget->isFP64bit()) {
        SDValue Zero = CurDAG->getCopyFromReg(CurDAG->getEntryNode(), DL,
                                              RISCV64::ZERO, MVT::i32);
        ReplaceNode(Node, CurDAG->getMachineNode(RISCV64::BuildPairF64_64, DL,
                                                 MVT::f64, Zero, Zero));
      } else {
        SDValue Zero = CurDAG->getCopyFromReg(CurDAG->getEntryNode(), DL,
                                              RISCV64::ZERO, MVT::i32);
        ReplaceNode(Node, CurDAG->getMachineNode(RISCV64::BuildPairF64, DL,
                                                 MVT::f64, Zero, Zero));
      }
      return true;
    }
  ...
  case ISD::Constant: {
    ...
    // operand.
    if (Inst->Opc == RISCV64::LUi64)
      RegOpnd = CurDAG->getMachineNode(Inst->Opc, DL, MVT::i64, ImmOpnd);
    else
      RegOpnd =
        CurDAG->getMachineNode(Inst->Opc, DL, MVT::i64,
                               CurDAG->getRegister(RISCV64::ZERO_64, MVT::i64),
                               ImmOpnd);
  ...
  case ISD::INTRINSIC_WO_CHAIN: {
    ...
    case Intrinsic::mips_move_v:
      ...
      // unnecessary.
      ReplaceNode(Node, CurDAG->getMachineNode(RISCV64::MOVE_V, DL,
                                               Node->getValueType(0),
                                               Node->getOperand(1)));
      return true;
    }
  ...
  // Manually match RISCV64ISD::Ins nodes to get the correct instruction. It has
  ...
  case RISCV64ISD::Ins: {
    ...
    if (ResTy == MVT::i32) {
      ...
      if (Pos + Size <= 32)
        ...
        Opcode = RISCV64::DINSM;
      else
        Opcode = RISCV64::DINSU;
    }
  ...
  case RISCV64ISD::ThreadPointer: {
    EVT PtrVT = getTargetLowering()->getPointerTy(CurDAG->getDataLayout());
    unsigned RdhwrOpc, DestReg;

    if (PtrVT == MVT::i32) {
      RdhwrOpc = RISCV64::RDHWR;
      DestReg = RISCV64::V1;
    } else {
      RdhwrOpc = RISCV64::RDHWR64;
      DestReg = RISCV64::V1_64;
    }

    SDNode *Rdhwr =
        CurDAG->getMachineNode(RdhwrOpc, DL, Node->getValueType(0),
                               CurDAG->getRegister(RISCV64::HWR29, MVT::i32),
  ...
  case ISD::BUILD_VECTOR: {
    ...
    const RISCV64ABIInfo &ABI =
        static_cast<const RISCV64TargetMachine &>(TM).getABI();
    ...
    if (SplatValue.isSignedIntN(10)) {
      ...
      const unsigned ADDiuOp = Is32BitSplat ? RISCV64::ADDiu : RISCV64::DADDiu;
      const MVT SplatMVT = Is32BitSplat ? MVT::i32 : MVT::i64;
      SDValue ZeroVal = CurDAG->getRegister(
          Is32BitSplat ? RISCV64::ZERO : RISCV64::ZERO_64, SplatMVT);

      const unsigned FILLOp =
          SplatBitSize == 16
              ? RISCV64::FILL_H
              : (SplatBitSize == 32 ? RISCV64::FILL_W
                                    : (SplatBitSize == 64 ? RISCV64::FILL_D : 0));

      assert(FILLOp != 0 && "Unknown FILL Op for splat synthesis!");
      assert((!ABI.IsO32() || (FILLOp != RISCV64::FILL_D)) &&
      ...
      SDValue ZeroVal = CurDAG->getRegister(RISCV64::ZERO, MVT::i32);

      SDValue LoVal = CurDAG->getTargetConstant(Lo, DL, MVT::i32);
      SDValue HiVal = CurDAG->getTargetConstant(Hi, DL, MVT::i32);

      if (Hi)
        Res = CurDAG->getMachineNode(RISCV64::LUi, DL, MVT::i32, HiVal);

      if (Lo)
        Res = CurDAG->getMachineNode(RISCV64::ORi, DL, MVT::i32,
                                     Hi ? SDValue(Res, 0) : ZeroVal, LoVal);

      assert((Hi || Lo) && "Zero case reached 32 bit case splat synthesis!");
      Res = CurDAG->getMachineNode(RISCV64::FILL_W, DL, MVT::v4i32, SDValue(Res, 0));
      ...
      Res = CurDAG->getMachineNode(
              RISCV64::SUBREG_TO_REG, DL, MVT::i64,
              CurDAG->getTargetConstant(((Hi >> 15) & 0x1), DL, MVT::i64),
              SDValue(Res, 0),
              CurDAG->getTargetConstant(RISCV64::sub_32, DL, MVT::i64));

      Res =
          CurDAG->getMachineNode(RISCV64::FILL_D, DL, MVT::v2i64, SDValue(Res, 0));
      ...
      SDNode *HiRes;
      if (Highest)
        HiRes = CurDAG->getMachineNode(RISCV64::LUi, DL, MVT::i32, HighestVal);

      if (Higher)
        HiRes = CurDAG->getMachineNode(RISCV64::ORi, DL, MVT::i32,
                                       Highest ? SDValue(HiRes, 0) : ZeroVal,
                                       HigherVal);


      if (ABI.IsO32()) {
        Res = CurDAG->getMachineNode(RISCV64::FILL_W, DL, MVT::v4i32,
                                     (Hi || Lo) ? SDValue(Res, 0) : ZeroVal);

        Res = CurDAG->getMachineNode(
            RISCV64::INSERT_W, DL, MVT::v4i32, SDValue(Res, 0),
        ...
        Res = CurDAG->getMachineNode(
            RISCV64::COPY_TO_REGCLASS, DL, ViaVecTy, SDValue(Res, 0),
            CurDAG->getTargetConstant(RC->getID(), DL, MVT::i32));

        Res = CurDAG->getMachineNode(
            RISCV64::SPLATI_D, DL, MVT::v2i64, SDValue(Res, 0),
            CurDAG->getTargetConstant(0, DL, MVT::i32));
      } else if (ABI.IsN64() || ABI.IsN32()) {

        SDValue Zero64Val = CurDAG->getRegister(RISCV64::ZERO_64, MVT::i64);
        ...
        if (ResNonZero) {
          ...
          Res = CurDAG->getMachineNode(RISCV64::DINSU, DL, MVT::i64, Ops);
        } else if (HiResNonZero) {
          Res = CurDAG->getMachineNode(
              RISCV64::DSLL32, DL, MVT::i64, SDValue(HiRes, 0),
              CurDAG->getTargetConstant(0, DL, MVT::i32));
        } else
          llvm_unreachable(
              "Zero splat value handled by non-zero 64bit splat synthesis!");

        Res = CurDAG->getMachineNode(RISCV64::FILL_D, DL, MVT::v2i64, SDValue(Res, 0));
      } else
        llvm_unreachable("Unknown ABI in RISCV64ISelDAGToDAG!");
    ...
    if (ResVecTy != ViaVecTy) {
      ...
      Res = CurDAG->getMachineNode(RISCV64::COPY_TO_REGCLASS, DL,
...
FunctionPass *llvm::createRISCV64SEISelDag(RISCV64TargetMachine &TM,
                                        CodeGenOpt::Level OptLevel) {
  return new RISCV64SEDAGToDAGISel(TM, OptLevel);
}
