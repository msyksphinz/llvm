//===- RISCV64ISelLowering.h - RISCV64 DAG Lowering Interface ---------*- C++ -*-===//
//
...
class MachineInstr;
class RISCV64CCState;
class RISCV64FunctionInfo;
class RISCV64Subtarget;
class RISCV64TargetMachine;
class TargetLibraryInfo;
class TargetRegisterClass;

  namespace RISCV64ISD {
    ...
    enum NodeType : unsigned {
      ...
      // No relation with RISCV64 Hi register
      Hi,

      // Get the Lower 16 bits from a 32/64-bit immediate
      // No relation with RISCV64 Lo register
      Lo,
  ...
  } // ene namespace RISCV64ISD

  //===--------------------------------------------------------------------===//
  // TargetLowering Implementation
  //===--------------------------------------------------------------------===//

  class RISCV64TargetLowering : public TargetLowering  {
    bool isMicroRISCV64;

  public:
    explicit RISCV64TargetLowering(const RISCV64TargetMachine &TM,
                                const RISCV64Subtarget &STI);

    static const RISCV64TargetLowering *create(const RISCV64TargetMachine &TM,
                                            const RISCV64Subtarget &STI);
    ...
    getExceptionPointerRegister(const Constant *PersonalityFn) const override {
      return ABI.IsN64() ? RISCV64::A0_64 : RISCV64::A0;
    }
    ...
    getExceptionSelectorRegister(const Constant *PersonalityFn) const override {
      return ABI.IsN64() ? RISCV64::A1_64 : RISCV64::A1;
    }

    /// Returns true if a cast between SrcAS and DestAS is a noop.
    bool isNoopAddrSpaceCast(unsigned SrcAS, unsigned DestAS) const override {
      // RISCV64 doesn't have any special address spaces so we just reserve
  ...
  protected:
    ...
    SDValue getAddrLocal(NodeTy *N, const SDLoc &DL, EVT Ty, SelectionDAG &DAG,
      ...
      unsigned GOTFlag = IsN32OrN64 ? RISCV64II::MO_GOT_PAGE : RISCV64II::MO_GOT;
      SDValue GOT = DAG.getNode(RISCV64ISD::Wrapper, DL, Ty, getGlobalReg(DAG, Ty),
                                getTargetNode(N, Ty, DAG, GOTFlag));
      SDValue Load =
          DAG.getLoad(Ty, DL, DAG.getEntryNode(), GOT,
                      MachinePointerInfo::getGOT(DAG.getMachineFunction()));
      unsigned LoFlag = IsN32OrN64 ? RISCV64II::MO_GOT_OFST : RISCV64II::MO_ABS_LO;
      SDValue Lo = DAG.getNode(RISCV64ISD::Lo, DL, Ty,
    ...
    SDValue getAddrGlobal(NodeTy *N, const SDLoc &DL, EVT Ty, SelectionDAG &DAG,
      ...
      SDValue Tgt = DAG.getNode(RISCV64ISD::Wrapper, DL, Ty, getGlobalReg(DAG, Ty),
    ...
    SDValue getAddrGlobalLargeGOT(NodeTy *N, const SDLoc &DL, EVT Ty,
                                  ...
                                  const MachinePointerInfo &PtrInfo) const {
      SDValue Hi = DAG.getNode(RISCV64ISD::GotHi, DL, Ty,
                               getTargetNode(N, Ty, DAG, HiFlag));
      Hi = DAG.getNode(ISD::ADD, DL, Ty, Hi, getGlobalReg(DAG, Ty));
      SDValue Wrapper = DAG.getNode(RISCV64ISD::Wrapper, DL, Ty, Hi,
                                    getTargetNode(N, Ty, DAG, LoFlag));
    ...
    SDValue getAddrNonPIC(NodeTy *N, const SDLoc &DL, EVT Ty,
      ...
      SDValue Hi = getTargetNode(N, Ty, DAG, RISCV64II::MO_ABS_HI);
      SDValue Lo = getTargetNode(N, Ty, DAG, RISCV64II::MO_ABS_LO);
      return DAG.getNode(ISD::ADD, DL, Ty,
                         DAG.getNode(RISCV64ISD::Hi, DL, Ty, Hi),
                         DAG.getNode(RISCV64ISD::Lo, DL, Ty, Lo));
   }
   ...
   SDValue getAddrNonPICSym64(NodeTy *N, const SDLoc &DL, EVT Ty,
      ...
      SDValue Highest =
          DAG.getNode(RISCV64ISD::Highest, DL, Ty,
                      getTargetNode(N, Ty, DAG, RISCV64II::MO_HIGHEST));
      SDValue Higher = getTargetNode(N, Ty, DAG, RISCV64II::MO_HIGHER);
      SDValue HigherPart =
          DAG.getNode(ISD::ADD, DL, Ty, Highest,
                      DAG.getNode(RISCV64ISD::Higher, DL, Ty, Higher));
    ...
    SDValue getAddrGPRel(NodeTy *N, const SDLoc &DL, EVT Ty,
      ...
      SDValue GPRel = getTargetNode(N, Ty, DAG, RISCV64II::MO_GPREL);
      return DAG.getNode(
          ISD::ADD, DL, Ty,
          DAG.getRegister(IsN64 ? RISCV64::GP_64 : RISCV64::GP, Ty),
          DAG.getNode(RISCV64ISD::GPRel, DL, DAG.getVTList(Ty), GPRel));
    }
  ...
  private:
    ...
    isEligibleForTailCallOptimization(const CCState &CCInfo,
                                      ...
                                      const RISCV64FunctionInfo &FI) const = 0;
  ...
  /// Create RISCV64TargetLowering objects.
  const RISCV64TargetLowering *
  createRISCV6416TargetLowering(const RISCV64TargetMachine &TM,
                             const RISCV64Subtarget &STI);
  const RISCV64TargetLowering *
  createRISCV64SETargetLowering(const RISCV64TargetMachine &TM,
                             const RISCV64Subtarget &STI);
