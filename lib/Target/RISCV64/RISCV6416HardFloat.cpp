//===- RISCV6416HardFloat.cpp for RISCV6416 Hard Float --------------------------===//
//
...
//===----------------------------------------------------------------------===//
//
// This file defines a pass needed for RISCV6416 Hard Float
//
//===----------------------------------------------------------------------===//
...
namespace {
  ...
  class RISCV6416HardFloat : public ModulePass {
  public:
    static char ID;

    RISCV6416HardFloat() : ModulePass(ID) {}
...
// We swap between FP and Integer registers to allow RISCV6416 and RISCV6432 to
// interoperate
  ...
  for (auto &BB: F)
    for (auto &I: BB) {
      if (const ReturnInst *RI = dyn_cast<ReturnInst>(&I)) {
        ...
        // this __RISCV6416RetHelper indicates that so that later
        // during call setup, the proper call lowering to the helper
        // functions will take place.
        //
        A = A.addAttribute(C, AttributeList::FunctionIndex,
                           "__RISCV6416RetHelper");
        A = A.addAttribute(C, AttributeList::FunctionIndex,
                           Attribute::ReadNone);
        A = A.addAttribute(C, AttributeList::FunctionIndex,
                           Attribute::NoInline);
...
bool RISCV6416HardFloat::runOnModule(Module &M) {
  auto &TM = static_cast<const RISCV64TargetMachine &>(
      getAnalysis<TargetPassConfig>().getTM<TargetMachine>());
  LLVM_DEBUG(errs() << "Run on Module RISCV6416HardFloat\n");
  bool Modified = false;
...
ModulePass *llvm::createRISCV6416HardFloatPass() {
  return new RISCV6416HardFloat();
}
