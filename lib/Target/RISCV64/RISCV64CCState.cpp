//===---- RISCV64CCState.cpp - CCState with RISCV64 specific extensions ---------===//
//
...
RISCV64CCState::SpecialCallingConvType
RISCV64CCState::getSpecialCallingConvForCallee(const SDNode *Callee,
                                            const RISCV64Subtarget &Subtarget) {
  RISCV64CCState::SpecialCallingConvType SpecialCallingConv = NoSpecialCallingConv;
  if (Subtarget.inRISCV6416HardFloat()) {
    if (const GlobalAddressSDNode *G =
      ...
      if (F && F->hasFnAttribute("__RISCV6416RetHelper")) {
        SpecialCallingConv = RISCV6416RetHelperConv;
      }
    }
  }
...
void RISCV64CCState::PreAnalyzeCallResultForF128(
...
/// record this for use by RetCC_RISCV64N.
void RISCV64CCState::PreAnalyzeReturnForF128(
...
/// this.
void RISCV64CCState::PreAnalyzeCallResultForVectorFloat(
...
/// this.
void RISCV64CCState::PreAnalyzeReturnForVectorFloat(
    const SmallVectorImpl<ISD::OutputArg> &Outs) {
  for (unsigned i = 0; i < Outs.size(); ++i) {
...
void RISCV64CCState::PreAnalyzeCallOperands(
    const SmallVectorImpl<ISD::OutputArg> &Outs,
...
/// and record this.
void RISCV64CCState::PreAnalyzeFormalArgumentsForF128(
    const SmallVectorImpl<ISD::InputArg> &Ins) {
  const MachineFunction &MF = getMachineFunction();
  for (unsigned i = 0; i < Ins.size(); ++i) {
