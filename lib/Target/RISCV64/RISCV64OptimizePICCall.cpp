//===- RISCV64OptimizePICCall.cpp - Optimize PIC Calls -----------------------===//
//
...
public:
  ...
  StringRef getPassName() const override { return "RISCV64 OptimizePICCall"; }
...
static void setCallTargetReg(MachineBasicBlock *MBB,
  ...
  unsigned DstReg = getRegTy(SrcReg, MF) == MVT::i32 ? RISCV64::T9 : RISCV64::T9_64;
...
static void eraseGPOpnd(MachineInstr &MI) {
  ...
  unsigned Reg = Ty == MVT::i32 ? RISCV64::GP : RISCV64::GP_64;
...
bool OptimizePICCall::runOnMachineFunction(MachineFunction &F) {
  if (static_cast<const RISCV64Subtarget &>(F.getSubtarget()).inRISCV6416Mode())
    return false;
...
bool OptimizePICCall::isCallViaRegister(MachineInstr &MI, unsigned &Reg,
  ...
  if (Flags != RISCV64II::MO_GOT_CALL && Flags != RISCV64II::MO_CALL_LO16)
    return true;
...
FunctionPass *llvm::createRISCV64OptimizePICCallPass() {
