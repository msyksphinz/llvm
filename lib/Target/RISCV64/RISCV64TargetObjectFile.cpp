//===-- RISCV64TargetObjectFile.cpp - RISCV64 Object Files ----------------------===//
//
...
void RISCV64TargetObjectFile::Initialize(MCContext &Ctx, const TargetMachine &TM){
  ...
  SmallBSSSection = getContext().getELFSection(".sbss", ELF::SHT_NOBITS,
                                               ELF::SHF_WRITE | ELF::SHF_ALLOC |
                                                   ELF::SHF_MIPS_GPREL);
  this->TM = &static_cast<const RISCV64TargetMachine &>(TM);
}
...
/// section.
bool RISCV64TargetObjectFile::IsGlobalInSmallSection(
...
IsGlobalInSmallSectionImpl(const GlobalObject *GO,
  ...
  const RISCV64Subtarget &Subtarget =
      *static_cast<const RISCV64TargetMachine &>(TM).getSubtargetImpl();
...
MCSection *RISCV64TargetObjectFile::SelectSectionForGlobal(
...
bool RISCV64TargetObjectFile::IsConstantInSmallSection(
    const DataLayout &DL, const Constant *CN, const TargetMachine &TM) const {
  return (static_cast<const RISCV64TargetMachine &>(TM)
              .getSubtargetImpl()
              ->useSmallSection() &&
...
/// Return true if this constant should be placed into small data section.
MCSection *RISCV64TargetObjectFile::getSectionForConstant(const DataLayout &DL,
                                                       SectionKind Kind,
                                                       const Constant *C,
...
const MCExpr *
RISCV64TargetObjectFile::getDebugThreadLocalSymbol(const MCSymbol *Sym) const {
  const MCExpr *Expr =
