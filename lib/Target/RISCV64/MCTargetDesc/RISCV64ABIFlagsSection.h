//===- RISCV64ABIFlagsSection.h - RISCV64 ELF ABI Flags Section -------*- C++ -*-===//
//
...
struct RISCV64ABIFlagsSection {
  ...
  RISCV64::AFL_REG GPRSize = RISCV64::AFL_REG_NONE;
  // The size of co-processor 1 registers.
  RISCV64::AFL_REG CPR1Size = RISCV64::AFL_REG_NONE;
  // The size of co-processor 2 registers.
  RISCV64::AFL_REG CPR2Size = RISCV64::AFL_REG_NONE;
  // Processor-specific extension.
  RISCV64::AFL_EXT ISAExtension = RISCV64::AFL_EXT_NONE;
  // Mask of ASEs used.
  uint32_t ASESet = 0;
...
public:
  RISCV64ABIFlagsSection() = default;
  ...
  uint32_t getFlags1Value() {
    ...
    if (OddSPReg)
      Value |= (uint32_t)RISCV64::AFL_FLAGS1_ODDSPREG;
  ...
  void setISALevelAndRevisionFromPredicates(const PredicateLibrary &P) {
    if (P.hasRISCV6464()) {
      ...
      if (P.hasRISCV6464r6())
        ISARevision = 6;
      else if (P.hasRISCV6464r5())
        ISARevision = 5;
      else if (P.hasRISCV6464r3())
        ISARevision = 3;
      else if (P.hasRISCV6464r2())
        ISARevision = 2;
      else
        ISARevision = 1;
    } else if (P.hasRISCV6432()) {
      ISALevel = 32;
      if (P.hasRISCV6432r6())
        ISARevision = 6;
      else if (P.hasRISCV6432r5())
        ISARevision = 5;
      else if (P.hasRISCV6432r3())
        ISARevision = 3;
      else if (P.hasRISCV6432r2())
        ISARevision = 2;
      else
        ISARevision = 1;
    } else {
      ...
      if (P.hasRISCV645())
        ISALevel = 5;
      else if (P.hasRISCV644())
        ISALevel = 4;
      else if (P.hasRISCV643())
        ISALevel = 3;
      else if (P.hasRISCV642())
        ISALevel = 2;
      else if (P.hasRISCV641())
        ISALevel = 1;
      else
  ...
  void setGPRSizeFromPredicates(const PredicateLibrary &P) {
    GPRSize = P.isGP64bit() ? RISCV64::AFL_REG_64 : RISCV64::AFL_REG_32;
  }

  template <class PredicateLibrary>
  void setCPR1SizeFromPredicates(const PredicateLibrary &P) {
    if (P.useSoftFloat())
      CPR1Size = RISCV64::AFL_REG_NONE;
    else if (P.hasMSA())
      CPR1Size = RISCV64::AFL_REG_128;
    else
      CPR1Size = P.isFP64bit() ? RISCV64::AFL_REG_64 : RISCV64::AFL_REG_32;
  }

  template <class PredicateLibrary>
  void setISAExtensionFromPredicates(const PredicateLibrary &P) {
    if (P.hasCnRISCV64())
      ISAExtension = RISCV64::AFL_EXT_OCTEON;
    else
      ISAExtension = RISCV64::AFL_EXT_NONE;
  }
  ...
  void setASESetFromPredicates(const PredicateLibrary &P) {
    ASESet = 0;
    if (P.hasDSP())
      ASESet |= RISCV64::AFL_ASE_DSP;
    if (P.hasDSPR2())
      ASESet |= RISCV64::AFL_ASE_DSPR2;
    if (P.hasMSA())
      ASESet |= RISCV64::AFL_ASE_MSA;
    if (P.inMicroRISCV64Mode())
      ASESet |= RISCV64::AFL_ASE_MICROMIPS;
    if (P.inRISCV6416Mode())
      ASESet |= RISCV64::AFL_ASE_MIPS16;
    if (P.hasMT())
      ASESet |= RISCV64::AFL_ASE_MT;
    if (P.hasCRC())
      ASESet |= RISCV64::AFL_ASE_CRC;
    if (P.hasVirt())
      ASESet |= RISCV64::AFL_ASE_VIRT;
    if (P.hasGINV())
      ASESet |= RISCV64::AFL_ASE_GINV;
  }
