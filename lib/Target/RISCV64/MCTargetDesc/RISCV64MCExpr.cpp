//===-- RISCV64MCExpr.cpp - RISCV64 specific MC expression classes --------------===//
//
...
const RISCV64MCExpr *RISCV64MCExpr::create(RISCV64MCExpr::RISCV64ExprKind Kind,
                                     const MCExpr *Expr, MCContext &Ctx) {
  return new (Ctx) RISCV64MCExpr(Kind, Expr);
}

const RISCV64MCExpr *RISCV64MCExpr::createGpOff(RISCV64MCExpr::RISCV64ExprKind Kind,
                                          const MCExpr *Expr, MCContext &Ctx) {
  return create(Kind, create(MEK_NEG, create(MEK_GPREL, Expr, Ctx), Ctx), Ctx);
}

void RISCV64MCExpr::printImpl(raw_ostream &OS, const MCAsmInfo *MAI) const {
  int64_t AbsVal;
...
bool
RISCV64MCExpr::evaluateAsRelocatableImpl(MCValue &Res,
                                      const MCAsmLayout *Layout,
                                      const MCFixup *Fixup) const {
  // Look for the %hi(%neg(%gp_rel(X))) and %lo(%neg(%gp_rel(X))) special cases.
  if (isGpOff()) {
    const MCExpr *SubExpr =
        cast<RISCV64MCExpr>(cast<RISCV64MCExpr>(getSubExpr())->getSubExpr())
            ->getSubExpr();
...
void RISCV64MCExpr::visitUsedExpr(MCStreamer &Streamer) const {
...
static void fixELFSymbolsInTLSFixupsImpl(const MCExpr *Expr, MCAssembler &Asm) {
  switch (Expr->getKind()) {
  case MCExpr::Target:
    fixELFSymbolsInTLSFixupsImpl(cast<RISCV64MCExpr>(Expr)->getSubExpr(), Asm);
    break;
...
void RISCV64MCExpr::fixELFSymbolsInTLSFixups(MCAssembler &Asm) const {
  switch (getKind()) {
  case MEK_None:
  case MEK_Special:
    llvm_unreachable("MEK_None and MEK_Special are invalid");
  ...
  case MEK_PCREL_LO16:
    ...
    // a consecutive chain.
    if (const RISCV64MCExpr *E = dyn_cast<const RISCV64MCExpr>(getSubExpr()))
...
bool RISCV64MCExpr::isGpOff(RISCV64ExprKind &Kind) const {
  if (getKind() == MEK_HI || getKind() == MEK_LO) {
    if (const RISCV64MCExpr *S1 = dyn_cast<const RISCV64MCExpr>(getSubExpr())) {
      if (const RISCV64MCExpr *S2 = dyn_cast<const RISCV64MCExpr>(S1->getSubExpr())) {
