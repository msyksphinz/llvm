//===-- RISCV64ELFObjectWriter.cpp - RISCV64 ELF Writer -------------------------===//
//
...
#ifndef NDEBUG
raw_ostream &operator<<(raw_ostream &OS, const RISCV64RelocationEntry &RHS) {
  RHS.print(OS);
  return OS;
}
#endif

class RISCV64ELFObjectWriter : public MCELFObjectTargetWriter {
public:
  RISCV64ELFObjectWriter(uint8_t OSABI, bool HasRelocationAddend, bool Is64);

  ~RISCV64ELFObjectWriter() override = default;
...
static FindBestPredicateResult isMatchingReloc(const RISCV64RelocationEntry &X,
...
/// - It is not already involved in a match.
static bool compareMatchingRelocs(const RISCV64RelocationEntry &Candidate,
                                  const RISCV64RelocationEntry &PreviousBest) {
...
RISCV64ELFObjectWriter::RISCV64ELFObjectWriter(uint8_t OSABI,
                                         bool HasRelocationAddend, bool Is64)
    : MCELFObjectTargetWriter(Is64, OSABI, ELF::EM_MIPS, HasRelocationAddend) {}

unsigned RISCV64ELFObjectWriter::getRelocType(MCContext &Ctx,
  ...
  switch (Kind) {
  case RISCV64::fixup_RISCV64_NONE:
    return ELF::R_MIPS_NONE;
  case FK_Data_1:
  ...
  if (IsPCRel) {
    switch (Kind) {
    case RISCV64::fixup_RISCV64_Branch_PCRel:
...
void RISCV64ELFObjectWriter::sortRelocs(const MCAssembler &Asm,
  ...
  std::list<RISCV64RelocationEntry> Sorted;
  ...
  for (auto &R : Remainder) {
    ...
    auto InsertionPoint =
        find_best(Sorted.begin(), Sorted.end(),
                  [&R, &MatchingType](const RISCV64RelocationEntry &X) {
...
bool RISCV64ELFObjectWriter::needsRelocateWithSymbol(const MCSymbol &Sym,
                                                  unsigned Type) const {
...
llvm::createRISCV64ELFObjectWriter(const Triple &TT, bool IsN32) {
  uint8_t OSABI = MCELFObjectTargetWriter::getOSABI(TT.getOS());
  bool IsN64 = TT.isArch64Bit() && !IsN32;
  bool HasRelocationAddend = TT.isArch64Bit();
  return llvm::make_unique<RISCV64ELFObjectWriter>(OSABI, HasRelocationAddend,
                                                IsN64);
}
