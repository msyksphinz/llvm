//===- RISCV64OptionRecord.cpp - Abstraction for storing information ---------===//
//
...
void RISCV64RegInfoRecord::EmitRISCV64OptionRecord() {
  MCAssembler &MCA = Streamer->getAssembler();
  RISCV64TargetStreamer *MTS =
      static_cast<RISCV64TargetStreamer *>(Streamer->getTargetStreamer());

  Streamer->PushSection();

  // We need to distinguish between N64 and the rest because at the moment
  // we don't emit .RISCV64.options for other ELFs other than N64.
  // Since .reginfo has the same information as .RISCV64.options (ODK_REGINFO),
  // we can use the same abstraction (RISCV64RegInfoRecord class) to handle both.
  if (MTS->getABI().IsN64()) {
...
void RISCV64RegInfoRecord::SetPhysRegUsed(unsigned Reg,
