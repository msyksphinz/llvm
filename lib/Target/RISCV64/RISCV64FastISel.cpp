//===- RISCV64FastISel.cpp - RISCV64 FastISel implementation --------------------===//
//
...
/// RISCV64GenFastISel.inc, which is #included here.
///
//===----------------------------------------------------------------------===//
...
class RISCV64FastISel final : public FastISel {
  ...
  /// Subtarget - Keep a pointer to the RISCV64Subtarget around so that we can
...
private:
  ...
  const RISCV64ABIInfo &getABI() const {
    return static_cast<const RISCV64TargetMachine &>(TM).getABI();
  }

public:
  // Backend specific FastISel code.
  explicit RISCV64FastISel(FunctionLoweringInfo &funcInfo,
                        const TargetLibraryInfo *libInfo)
      : FastISel(funcInfo, libInfo), TM(funcInfo.MF->getTarget()),
        Subtarget(&funcInfo.MF->getSubtarget<RISCV64Subtarget>()),
        TII(*Subtarget->getInstrInfo()), TLI(*Subtarget->getTargetLowering()) {
    MFI = funcInfo.MF->getInfo<RISCV64FunctionInfo>();
...
#include "RISCV64GenFastISel.inc"
};

} // end anonymous namespace

static bool CC_RISCV64(unsigned ValNo, MVT ValVT, MVT LocVT,
                    CCValAssign::LocInfo LocInfo, ISD::ArgFlagsTy ArgFlags,
                    CCState &State) LLVM_ATTRIBUTE_UNUSED;

static bool CC_RISCV64O32_FP32(unsigned ValNo, MVT ValVT, MVT LocVT,
                            CCValAssign::LocInfo LocInfo,
                            ISD::ArgFlagsTy ArgFlags, CCState &State) {
  llvm_unreachable("should not be called");
}

static bool CC_RISCV64O32_FP64(unsigned ValNo, MVT ValVT, MVT LocVT,
                            CCValAssign::LocInfo LocInfo,
                            ISD::ArgFlagsTy ArgFlags, CCState &State) {
  llvm_unreachable("should not be called");
}
...
CCAssignFn *RISCV64FastISel::CCAssignFnForCall(CallingConv::ID CC) const {
  return CC_RISCV64O32;
}

unsigned RISCV64FastISel::emitLogicalOp(unsigned ISDOpc, MVT RetVT,
  ...
  unsigned ResultReg = createResultReg(&RISCV64::GPR32RegClass);
  if (!ResultReg)
    return 0;
...
unsigned RISCV64FastISel::fastMaterializeAlloca(const AllocaInst *AI) {
  ...
  if (SI != FuncInfo.StaticAllocaMap.end()) {
    unsigned ResultReg = createResultReg(&RISCV64::GPR32RegClass);
    BuildMI(*FuncInfo.MBB, FuncInfo.InsertPt, DbgLoc, TII.get(RISCV64::LEA_ADDiu),
            ResultReg)
...
unsigned RISCV64FastISel::materializeInt(const Constant *C, MVT VT) {
...
unsigned RISCV64FastISel::materialize32BitInt(int64_t Imm,
                                           const TargetRegisterClass *RC) {
  unsigned ResultReg = createResultReg(RC);

  if (isInt<16>(Imm)) {
    unsigned Opc = RISCV64::ADDiu;
    emitInst(Opc, ResultReg).addReg(RISCV64::ZERO).addImm(Imm);
    return ResultReg;
  } else if (isUInt<16>(Imm)) {
    emitInst(RISCV64::ORi, ResultReg).addReg(RISCV64::ZERO).addImm(Imm);
    return ResultReg;
  }
  ...
  if (Lo) {
    ...
    emitInst(RISCV64::LUi, TmpReg).addImm(Hi);
    emitInst(RISCV64::ORi, ResultReg).addReg(TmpReg).addImm(Lo);
  } else {
    emitInst(RISCV64::LUi, ResultReg).addImm(Hi);
  }
  return ResultReg;
}

unsigned RISCV64FastISel::materializeFP(const ConstantFP *CFP, MVT VT) {
  if (UnsupportedFPMode)
    return 0;
  int64_t Imm = CFP->getValueAPF().bitcastToAPInt().getZExtValue();
  if (VT == MVT::f32) {
    const TargetRegisterClass *RC = &RISCV64::FGR32RegClass;
    unsigned DestReg = createResultReg(RC);
    unsigned TempReg = materialize32BitInt(Imm, &RISCV64::GPR32RegClass);
    emitInst(RISCV64::MTC1, DestReg).addReg(TempReg);
    return DestReg;
  } else if (VT == MVT::f64) {
    const TargetRegisterClass *RC = &RISCV64::AFGR64RegClass;
    unsigned DestReg = createResultReg(RC);
    unsigned TempReg1 = materialize32BitInt(Imm >> 32, &RISCV64::GPR32RegClass);
    unsigned TempReg2 =
        materialize32BitInt(Imm & 0xFFFFFFFF, &RISCV64::GPR32RegClass);
    emitInst(RISCV64::BuildPairF64, DestReg).addReg(TempReg2).addReg(TempReg1);
    return DestReg;
  }
  return 0;
}

unsigned RISCV64FastISel::materializeGV(const GlobalValue *GV, MVT VT) {
  // For now 32-bit only.
  if (VT != MVT::i32)
    return 0;
  const TargetRegisterClass *RC = &RISCV64::GPR32RegClass;
  ...
  if (IsThreadLocal)
    return 0;
  emitInst(RISCV64::LW, DestReg)
      .addReg(MFI->getGlobalBaseReg())
      .addGlobalAddress(GV, 0, RISCV64II::MO_GOT);
  if ((GV->hasInternalLinkage() ||
       (GV->hasLocalLinkage() && !isa<Function>(GV)))) {
    unsigned TempReg = createResultReg(RC);
    emitInst(RISCV64::ADDiu, TempReg)
        .addReg(DestReg)
        .addGlobalAddress(GV, 0, RISCV64II::MO_ABS_LO);
    DestReg = TempReg;
  }
  return DestReg;
}

unsigned RISCV64FastISel::materializeExternalCallSym(MCSymbol *Sym) {
  const TargetRegisterClass *RC = &RISCV64::GPR32RegClass;
  unsigned DestReg = createResultReg(RC);
  emitInst(RISCV64::LW, DestReg)
      .addReg(MFI->getGlobalBaseReg())
      .addSym(Sym, RISCV64II::MO_GOT);
  return DestReg;
}

// Materialize a constant into a register, and return the register
// number (or zero if we failed to handle it).
unsigned RISCV64FastISel::fastMaterializeConstant(const Constant *C) {
...
bool RISCV64FastISel::computeAddress(const Value *Obj, Address &Addr) {
...
bool RISCV64FastISel::computeCallAddress(const Value *V, Address &Addr) {
  const User *U = nullptr;
  unsigned Opcode = Instruction::UserOp1;
...
bool RISCV64FastISel::isTypeLegal(Type *Ty, MVT &VT) {
...
bool RISCV64FastISel::isTypeSupported(Type *Ty, MVT &VT) {
  if (Ty->isVectorTy())
    return false;
...
bool RISCV64FastISel::isLoadTypeLegal(Type *Ty, MVT &VT) {
  if (isTypeLegal(Ty, VT))
    return true;
...
//
bool RISCV64FastISel::emitCmp(unsigned ResultReg, const CmpInst *CI) {
  ...
  case CmpInst::ICMP_EQ: {
    unsigned TempReg = createResultReg(&RISCV64::GPR32RegClass);
    emitInst(RISCV64::XOR, TempReg).addReg(LeftReg).addReg(RightReg);
    emitInst(RISCV64::SLTiu, ResultReg).addReg(TempReg).addImm(1);
    break;
  }
  case CmpInst::ICMP_NE: {
    unsigned TempReg = createResultReg(&RISCV64::GPR32RegClass);
    emitInst(RISCV64::XOR, TempReg).addReg(LeftReg).addReg(RightReg);
    emitInst(RISCV64::SLTu, ResultReg).addReg(RISCV64::ZERO).addReg(TempReg);
    break;
  }
  case CmpInst::ICMP_UGT:
    emitInst(RISCV64::SLTu, ResultReg).addReg(RightReg).addReg(LeftReg);
    break;
  case CmpInst::ICMP_ULT:
    emitInst(RISCV64::SLTu, ResultReg).addReg(LeftReg).addReg(RightReg);
    break;
  case CmpInst::ICMP_UGE: {
    unsigned TempReg = createResultReg(&RISCV64::GPR32RegClass);
    emitInst(RISCV64::SLTu, TempReg).addReg(LeftReg).addReg(RightReg);
    emitInst(RISCV64::XORi, ResultReg).addReg(TempReg).addImm(1);
    break;
  }
  case CmpInst::ICMP_ULE: {
    unsigned TempReg = createResultReg(&RISCV64::GPR32RegClass);
    emitInst(RISCV64::SLTu, TempReg).addReg(RightReg).addReg(LeftReg);
    emitInst(RISCV64::XORi, ResultReg).addReg(TempReg).addImm(1);
    break;
  }
  case CmpInst::ICMP_SGT:
    emitInst(RISCV64::SLT, ResultReg).addReg(RightReg).addReg(LeftReg);
    break;
  case CmpInst::ICMP_SLT:
    emitInst(RISCV64::SLT, ResultReg).addReg(LeftReg).addReg(RightReg);
    break;
  case CmpInst::ICMP_SGE: {
    unsigned TempReg = createResultReg(&RISCV64::GPR32RegClass);
    emitInst(RISCV64::SLT, TempReg).addReg(LeftReg).addReg(RightReg);
    emitInst(RISCV64::XORi, ResultReg).addReg(TempReg).addImm(1);
    break;
  }
  case CmpInst::ICMP_SLE: {
    unsigned TempReg = createResultReg(&RISCV64::GPR32RegClass);
    emitInst(RISCV64::SLT, TempReg).addReg(RightReg).addReg(LeftReg);
    emitInst(RISCV64::XORi, ResultReg).addReg(TempReg).addImm(1);
    break;
  }
  ...
  case CmpInst::FCMP_OGE: {
    ...
    switch (P) {
    case CmpInst::FCMP_OEQ:
      Opc = IsFloat ? RISCV64::C_EQ_S : RISCV64::C_EQ_D32;
      CondMovOpc = RISCV64::MOVT_I;
      break;
    case CmpInst::FCMP_UNE:
      Opc = IsFloat ? RISCV64::C_EQ_S : RISCV64::C_EQ_D32;
      CondMovOpc = RISCV64::MOVF_I;
      break;
    case CmpInst::FCMP_OLT:
      Opc = IsFloat ? RISCV64::C_OLT_S : RISCV64::C_OLT_D32;
      CondMovOpc = RISCV64::MOVT_I;
      break;
    case CmpInst::FCMP_OLE:
      Opc = IsFloat ? RISCV64::C_OLE_S : RISCV64::C_OLE_D32;
      CondMovOpc = RISCV64::MOVT_I;
      break;
    case CmpInst::FCMP_OGT:
      Opc = IsFloat ? RISCV64::C_ULE_S : RISCV64::C_ULE_D32;
      CondMovOpc = RISCV64::MOVF_I;
      break;
    case CmpInst::FCMP_OGE:
      Opc = IsFloat ? RISCV64::C_ULT_S : RISCV64::C_ULT_D32;
      CondMovOpc = RISCV64::MOVF_I;
      break;
    default:
      llvm_unreachable("Only switching of a subset of CCs.");
    }
    unsigned RegWithZero = createResultReg(&RISCV64::GPR32RegClass);
    unsigned RegWithOne = createResultReg(&RISCV64::GPR32RegClass);
    emitInst(RISCV64::ADDiu, RegWithZero).addReg(RISCV64::ZERO).addImm(0);
    emitInst(RISCV64::ADDiu, RegWithOne).addReg(RISCV64::ZERO).addImm(1);
    emitInst(Opc).addReg(RISCV64::FCC0, RegState::Define).addReg(LeftReg)
                 .addReg(RightReg);
...
bool RISCV64FastISel::emitLoad(MVT VT, unsigned &ResultReg, Address &Addr,
                            unsigned Alignment) {
  //
  ...
  switch (VT.SimpleTy) {
  case MVT::i32:
    ResultReg = createResultReg(&RISCV64::GPR32RegClass);
    Opc = RISCV64::LW;
    break;
  case MVT::i16:
    ResultReg = createResultReg(&RISCV64::GPR32RegClass);
  ...
  case MVT::f32:
    if (UnsupportedFPMode)
      return false;
    ResultReg = createResultReg(&RISCV64::FGR32RegClass);
    Opc = RISCV64::LWC1;
    break;
  case MVT::f64:
    if (UnsupportedFPMode)
      return false;
    ResultReg = createResultReg(&RISCV64::AFGR64RegClass);
    Opc = RISCV64::LDC1;
    break;
  default:
    return false;
...
bool RISCV64FastISel::emitStore(MVT VT, unsigned SrcReg, Address &Addr,
                             unsigned Alignment) {
  //
  // more cases will be handled here in following patches.
  //
...
bool RISCV64FastISel::selectLogicalOp(const Instruction *I) {
  MVT VT;
...
bool RISCV64FastISel::selectLoad(const Instruction *I) {
...
bool RISCV64FastISel::selectStore(const Instruction *I) {
...
bool RISCV64FastISel::selectBranch(const Instruction *I) {
  ...
  if (const CmpInst *CI = dyn_cast<CmpInst>(BI->getCondition())) {
    ...
    BuildMI(*BrBB, FuncInfo.InsertPt, DbgLoc, TII.get(RISCV64::BGTZ))
        .addReg(CondReg)
        .addMBB(TBB);
...
bool RISCV64FastISel::selectCmp(const Instruction *I) {
...
bool RISCV64FastISel::selectFPExt(const Instruction *I) {
  if (UnsupportedFPMode)
    return false;
  ...
  unsigned DestReg = createResultReg(&RISCV64::AFGR64RegClass);
  emitInst(RISCV64::CVT_D32_S, DestReg).addReg(SrcReg);
  updateValueMap(I, DestReg);
  return true;
}

bool RISCV64FastISel::selectSelect(const Instruction *I) {
  ...
  if (VT.isInteger() && !VT.isVector() && VT.getSizeInBits() <= 32) {
    CondMovOpc = RISCV64::MOVN_I_I;
    RC = &RISCV64::GPR32RegClass;
  } else if (VT == MVT::f32) {
    CondMovOpc = RISCV64::MOVN_I_S;
    RC = &RISCV64::FGR32RegClass;
  } else if (VT == MVT::f64) {
    CondMovOpc = RISCV64::MOVN_I_D32;
    RC = &RISCV64::AFGR64RegClass;
  } else
    return false;
  ...
  unsigned ZExtCondReg = createResultReg(&RISCV64::GPR32RegClass);
  if (!ZExtCondReg)
    return false;
...
bool RISCV64FastISel::selectFPTrunc(const Instruction *I) {
  if (UnsupportedFPMode)
    return false;
  Value *Src = I->getOperand(0);
  EVT SrcVT = TLI.getValueType(DL, Src->getType(), true);
  ...
  unsigned DestReg = createResultReg(&RISCV64::FGR32RegClass);
  if (!DestReg)
    return false;

  emitInst(RISCV64::CVT_S_D32, DestReg).addReg(SrcReg);
  updateValueMap(I, DestReg);
  return true;
}

// Attempt to fast-select a floating-point-to-integer conversion.
bool RISCV64FastISel::selectFPToInt(const Instruction *I, bool IsSigned) {
  if (UnsupportedFPMode)
    return false;
  MVT DstVT, SrcVT;
  if (!IsSigned)
  ...
  unsigned DestReg = createResultReg(&RISCV64::GPR32RegClass);
  unsigned TempReg = createResultReg(&RISCV64::FGR32RegClass);
  unsigned Opc = (SrcVT == MVT::f32) ? RISCV64::TRUNC_W_S : RISCV64::TRUNC_W_D32;

  // Generate the convert.
  emitInst(Opc, TempReg).addReg(SrcReg);
  emitInst(RISCV64::MFC1, DestReg).addReg(TempReg);

  updateValueMap(I, DestReg);
  return true;
}

bool RISCV64FastISel::processCallArgs(CallLoweringInfo &CLI,
  ...
  emitInst(RISCV64::ADJCALLSTACKDOWN).addImm(16).addImm(0);
  // Process the args.
  MVT firstMVT;
  for (unsigned i = 0, e = ArgLocs.size(); i != e; ++i) {
    ...
    if (i == 0) {
      firstMVT = ArgVT;
      if (ArgVT == MVT::f32) {
        VA.convertToReg(RISCV64::F12);
      } else if (ArgVT == MVT::f64) {
        VA.convertToReg(RISCV64::D6);
      }
    } else if (i == 1) {
      if ((firstMVT == MVT::f32) || (firstMVT == MVT::f64)) {
        if (ArgVT == MVT::f32) {
          VA.convertToReg(RISCV64::F14);
        } else if (ArgVT == MVT::f64) {
          VA.convertToReg(RISCV64::D7);
        }
      }
    }
    if (((ArgVT == MVT::i32) || (ArgVT == MVT::f32) || (ArgVT == MVT::i16) ||
      ...
      case 0:
        VA.convertToReg(RISCV64::A0);
        break;
      case 4:
        VA.convertToReg(RISCV64::A1);
        break;
      case 8:
        VA.convertToReg(RISCV64::A2);
        break;
      case 12:
        VA.convertToReg(RISCV64::A3);
        break;
      default:
        break;
      }
    ...
    if (VA.isRegLoc() && !VA.needsCustom()) {
      ...
      llvm_unreachable("RISCV64 does not use custom args.");
      return false;
    } else {
      //
      // FIXME: This path will currently return false. It was copied
      // from the AArch64 port and should be essentially fine for RISCV64 too.
...
bool RISCV64FastISel::finishCall(CallLoweringInfo &CLI, MVT RetVT,
                              unsigned NumBytes) {
  CallingConv::ID CC = CLI.CallConv;
  emitInst(RISCV64::ADJCALLSTACKUP).addImm(16).addImm(0);
  if (RetVT != MVT::isVoid) {
    SmallVector<CCValAssign, 16> RVLocs;
    RISCV64CCState CCInfo(CC, false, *FuncInfo.MF, RVLocs, *Context);

    CCInfo.AnalyzeCallResult(CLI.Ins, RetCC_RISCV64, CLI.RetTy,
...
bool RISCV64FastISel::fastLowerArguments() {
  ...
  std::array<MCPhysReg, 4> GPR32ArgRegs = {{RISCV64::A0, RISCV64::A1, RISCV64::A2,
                                           RISCV64::A3}};
  std::array<MCPhysReg, 2> FGR32ArgRegs = {{RISCV64::F12, RISCV64::F14}};
  std::array<MCPhysReg, 2> AFGR64ArgRegs = {{RISCV64::D6, RISCV64::D7}};
  ...
  for (const auto &FormalArg : F->args()) {
    ...
    case MVT::i16:
      ...
      Allocation.emplace_back(&RISCV64::GPR32RegClass, *NextGPR32++);
    ...
    case MVT::f32:
      ...
      Allocation.emplace_back(&RISCV64::FGR32RegClass, *NextFGR32++);
    ...
    case MVT::f64:
      ...
      Allocation.emplace_back(&RISCV64::AFGR64RegClass, *NextAFGR64++);
      // Allocating an FGR32 also allocates the super-register AFGR64, and
  ...
  MF->getInfo<RISCV64FunctionInfo>()->setFormalArgInfo(IncomingArgSizeInBytes,
                                                    false);

  return true;
}

bool RISCV64FastISel::fastLowerCall(CallLoweringInfo &CLI) {
  CallingConv::ID CC = CLI.CallConv;
  ...
  emitInst(TargetOpcode::COPY, RISCV64::T9).addReg(DestAddress);
  MachineInstrBuilder MIB =
      BuildMI(*FuncInfo.MBB, FuncInfo.InsertPt, DbgLoc, TII.get(RISCV64::JALR),
              RISCV64::RA).addReg(RISCV64::T9);
...
bool RISCV64FastISel::fastLowerIntrinsicCall(const IntrinsicInst *II) {
  ...
  case Intrinsic::bswap: {
    ...
    if (VT == MVT::i16) {
      if (Subtarget->hasRISCV6432r2()) {
        emitInst(RISCV64::WSBH, DestReg).addReg(SrcReg);
        updateValueMap(II, DestReg);
        return true;
      } else {
        unsigned TempReg[3];
        for (int i = 0; i < 3; i++) {
          TempReg[i] = createResultReg(&RISCV64::GPR32RegClass);
          if (TempReg[i] == 0)
            return false;
        }
        emitInst(RISCV64::SLL, TempReg[0]).addReg(SrcReg).addImm(8);
        emitInst(RISCV64::SRL, TempReg[1]).addReg(SrcReg).addImm(8);
        emitInst(RISCV64::OR, TempReg[2]).addReg(TempReg[0]).addReg(TempReg[1]);
        emitInst(RISCV64::ANDi, DestReg).addReg(TempReg[2]).addImm(0xFFFF);
        updateValueMap(II, DestReg);
        return true;
      }
    } else if (VT == MVT::i32) {
      if (Subtarget->hasRISCV6432r2()) {
        unsigned TempReg = createResultReg(&RISCV64::GPR32RegClass);
        emitInst(RISCV64::WSBH, TempReg).addReg(SrcReg);
        emitInst(RISCV64::ROTR, DestReg).addReg(TempReg).addImm(16);
        updateValueMap(II, DestReg);
        return true;
      } else {
        unsigned TempReg[8];
        ...
        emitInst(RISCV64::SRL, TempReg[0]).addReg(SrcReg).addImm(8);
        emitInst(RISCV64::SRL, TempReg[1]).addReg(SrcReg).addImm(24);
        emitInst(RISCV64::ANDi, TempReg[2]).addReg(TempReg[0]).addImm(0xFF00);
        emitInst(RISCV64::OR, TempReg[3]).addReg(TempReg[1]).addReg(TempReg[2]);

        emitInst(RISCV64::ANDi, TempReg[4]).addReg(SrcReg).addImm(0xFF00);
        emitInst(RISCV64::SLL, TempReg[5]).addReg(TempReg[4]).addImm(8);

        emitInst(RISCV64::SLL, TempReg[6]).addReg(SrcReg).addImm(24);
        emitInst(RISCV64::OR, TempReg[7]).addReg(TempReg[3]).addReg(TempReg[5]);
        emitInst(RISCV64::OR, DestReg).addReg(TempReg[6]).addReg(TempReg[7]);
        updateValueMap(II, DestReg);
        return true;
      }
    }
...
bool RISCV64FastISel::selectRet(const Instruction *I) {
  ...
  if (Ret->getNumOperands() > 0) {
    ...
    RISCV64CCState CCInfo(CC, F.isVarArg(), *FuncInfo.MF, ValLocs,
                       I->getContext());
    CCAssignFn *RetCC = RetCC_RISCV64;
  ...
  MachineInstrBuilder MIB = emitInst(RISCV64::RetRA);
  for (unsigned i = 0, e = RetRegs.size(); i != e; ++i)
    MIB.addReg(RetRegs[i], RegState::Implicit);
  return true;
}

bool RISCV64FastISel::selectTrunc(const Instruction *I) {
...
bool RISCV64FastISel::selectIntExt(const Instruction *I) {
...
bool RISCV64FastISel::emitIntSExt32r1(MVT SrcVT, unsigned SrcReg, MVT DestVT,
                                   unsigned DestReg) {
  unsigned ShiftAmt;
  ...
  unsigned TempReg = createResultReg(&RISCV64::GPR32RegClass);
  emitInst(RISCV64::SLL, TempReg).addReg(SrcReg).addImm(ShiftAmt);
  emitInst(RISCV64::SRA, DestReg).addReg(TempReg).addImm(ShiftAmt);
  return true;
}

bool RISCV64FastISel::emitIntSExt32r2(MVT SrcVT, unsigned SrcReg, MVT DestVT,
                                   unsigned DestReg) {
  switch (SrcVT.SimpleTy) {
  default:
    return false;
  case MVT::i8:
    emitInst(RISCV64::SEB, DestReg).addReg(SrcReg);
    break;
  case MVT::i16:
    emitInst(RISCV64::SEH, DestReg).addReg(SrcReg);
    break;
  }
  return true;
}

bool RISCV64FastISel::emitIntSExt(MVT SrcVT, unsigned SrcReg, MVT DestVT,
                               unsigned DestReg) {
...
bool RISCV64FastISel::emitIntZExt(MVT SrcVT, unsigned SrcReg, MVT DestVT,
                               unsigned DestReg) {
  int64_t Imm;
  ...
  emitInst(RISCV64::ANDi, DestReg).addReg(SrcReg).addImm(Imm);
  return true;
}

bool RISCV64FastISel::emitIntExt(MVT SrcVT, unsigned SrcReg, MVT DestVT,
...
unsigned RISCV64FastISel::emitIntExt(MVT SrcVT, unsigned SrcReg, MVT DestVT,
                                  bool isZExt) {
  unsigned DestReg = createResultReg(&RISCV64::GPR32RegClass);
...
bool RISCV64FastISel::selectDivRem(const Instruction *I, unsigned ISDOpcode) {
  ...
  emitInst(RISCV64::TEQ).addReg(Src1Reg).addReg(RISCV64::ZERO).addImm(7);
...
bool RISCV64FastISel::selectShift(const Instruction *I) {
  MVT RetVT;
...
bool RISCV64FastISel::fastSelectInstruction(const Instruction *I) {
  switch (I->getOpcode()) {
  default:
    break;
...
unsigned RISCV64FastISel::getRegEnsuringSimpleIntegerWidening(const Value *V,
                                                           bool IsUnsigned) {
...
void RISCV64FastISel::simplifyAddress(Address &Addr) {
  if (!isInt<16>(Addr.getOffset())) {
    unsigned TempReg =
        materialize32BitInt(Addr.getOffset(), &RISCV64::GPR32RegClass);
    unsigned DestReg = createResultReg(&RISCV64::GPR32RegClass);
    emitInst(RISCV64::ADDu, DestReg).addReg(TempReg).addReg(Addr.getReg());
    Addr.setReg(DestReg);
    Addr.setOffset(0);
  }
}

unsigned RISCV64FastISel::fastEmitInst_rr(unsigned MachineInstOpcode,
                                       const TargetRegisterClass *RC,
                                       unsigned Op0, bool Op0IsKill,
                                       unsigned Op1, bool Op1IsKill) {
  ...
  if (MachineInstOpcode == RISCV64::MUL) {
    unsigned ResultReg = createResultReg(RC);
    ...
    BuildMI(*FuncInfo.MBB, FuncInfo.InsertPt, DbgLoc, II, ResultReg)
      ...
      .addReg(RISCV64::HI0, RegState::ImplicitDefine | RegState::Dead)
      .addReg(RISCV64::LO0, RegState::ImplicitDefine | RegState::Dead);
    return ResultReg;
  }
...
FastISel *RISCV64::createFastISel(FunctionLoweringInfo &funcInfo,
                               const TargetLibraryInfo *libInfo) {
  return new RISCV64FastISel(funcInfo, libInfo);
}
