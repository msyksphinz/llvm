...
RISCV64CallLowering::RISCV64CallLowering(const RISCV64TargetLowering &TLI)
    : CallLowering(&TLI) {}

bool RISCV64CallLowering::RISCV64Handler::assign(unsigned VReg,
...
bool RISCV64CallLowering::RISCV64Handler::assignVRegs(ArrayRef<unsigned> VRegs,
...
void RISCV64CallLowering::RISCV64Handler::setLeastSignificantFirst(
    SmallVectorImpl<unsigned> &VRegs) {
  if (!MIRBuilder.getMF().getDataLayout().isLittleEndian())
    std::reverse(VRegs.begin(), VRegs.end());
}

bool RISCV64CallLowering::RISCV64Handler::handle(
  ...
  const RISCV64TargetLowering &TLI = *static_cast<const RISCV64TargetLowering *>(
...
namespace {
class IncomingValueHandler : public RISCV64CallLowering::RISCV64Handler {
public:
  IncomingValueHandler(MachineIRBuilder &MIRBuilder, MachineRegisterInfo &MRI)
      : RISCV64Handler(MIRBuilder, MRI) {}
...
namespace {
class OutgoingValueHandler : public RISCV64CallLowering::RISCV64Handler {
public:
  OutgoingValueHandler(MachineIRBuilder &MIRBuilder, MachineRegisterInfo &MRI,
                       MachineInstrBuilder &MIB)
      : RISCV64Handler(MIRBuilder, MRI), MIB(MIB) {}
...
unsigned OutgoingValueHandler::getStackAddress(const CCValAssign &VA,
  ...
  MIRBuilder.buildCopy(SPReg, RISCV64::SP);
...
bool RISCV64CallLowering::lowerReturn(MachineIRBuilder &MIRBuilder,
                                   const Value *Val,
                                   ArrayRef<unsigned> VRegs) const {

  MachineInstrBuilder Ret = MIRBuilder.buildInstrNoInsert(RISCV64::RetRA);
  ...
  if (!VRegs.empty()) {
    ...
    const RISCV64TargetLowering &TLI = *getTLI<RISCV64TargetLowering>();
    ...
    RISCV64CCState CCInfo(F.getCallingConv(), F.isVarArg(), MF, ArgLocs,
                       F.getContext());
...
bool RISCV64CallLowering::lowerFormalArguments(MachineIRBuilder &MIRBuilder,
                                            const Function &F,
                                            ArrayRef<unsigned> VRegs) const {
  ...
  const RISCV64TargetMachine &TM =
      static_cast<const RISCV64TargetMachine &>(MF.getTarget());
  const RISCV64ABIInfo &ABI = TM.getABI();
...
bool RISCV64CallLowering::lowerCall(MachineIRBuilder &MIRBuilder,
  ...
  MachineInstrBuilder CallSeqStart =
      MIRBuilder.buildInstr(RISCV64::ADJCALLSTACKDOWN);
  ...
  MachineInstrBuilder MIB = MIRBuilder.buildInstrNoInsert(RISCV64::JAL);
  MIB.addDef(RISCV64::SP, RegState::Implicit);
  MIB.add(Callee);
  ...
  MIRBuilder.buildInstr(RISCV64::ADJCALLSTACKUP).addImm(NextStackOffset).addImm(0);

  return true;
}

template <typename T>
void RISCV64CallLowering::subTargetRegTypeForCallingConv(
...
void RISCV64CallLowering::splitToValueTypes(
