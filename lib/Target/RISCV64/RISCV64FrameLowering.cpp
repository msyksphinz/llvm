//===-- RISCV64FrameLowering.cpp - RISCV64 Frame Information --------------------===//
//
...
//===----------------------------------------------------------------------===//
//
// This file contains the RISCV64 implementation of TargetFrameLowering class.
//
//===----------------------------------------------------------------------===//
...
const RISCV64FrameLowering *RISCV64FrameLowering::create(const RISCV64Subtarget &ST) {
  if (ST.inRISCV6416Mode())
    return llvm::createRISCV6416FrameLowering(ST);

  return llvm::createRISCV64SEFrameLowering(ST);
}
...
bool RISCV64FrameLowering::hasFP(const MachineFunction &MF) const {
...
bool RISCV64FrameLowering::hasBP(const MachineFunction &MF) const {
  const MachineFrameInfo &MFI = MF.getFrameInfo();
  const TargetRegisterInfo *TRI = STI.getRegisterInfo();
...
uint64_t RISCV64FrameLowering::estimateStackSize(const MachineFunction &MF) const {
  const MachineFrameInfo &MFI = MF.getFrameInfo();
  const TargetRegisterInfo &TRI = *STI.getRegisterInfo();
...
MachineBasicBlock::iterator RISCV64FrameLowering::
eliminateCallFramePseudoInstr(MachineFunction &MF, MachineBasicBlock &MBB,
                              MachineBasicBlock::iterator I) const {
  unsigned SP = STI.getABI().IsN64() ? RISCV64::SP_64 : RISCV64::SP;

  if (!hasReservedCallFrame(MF)) {
    int64_t Amount = I->getOperand(0).getImm();
    if (I->getOpcode() == RISCV64::ADJCALLSTACKDOWN)
      Amount = -Amount;
