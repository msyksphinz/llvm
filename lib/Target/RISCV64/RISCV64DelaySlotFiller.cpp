//===- RISCV64DelaySlotFiller.cpp - RISCV64 Delay Slot Filler -------------------===//
//
...
static cl::opt<CompactBranchPolicy> RISCV64CompactBranchPolicy(
...
namespace {
  ...
  class RISCV64DelaySlotFiller : public MachineFunctionPass {
  public:
    RISCV64DelaySlotFiller() : MachineFunctionPass(ID) {
      initializeRISCV64DelaySlotFillerPass(*PassRegistry::getPassRegistry());
    }

    StringRef getPassName() const override { return "RISCV64 Delay Slot Filler"; }
  ...
  private:
    ...
    std::pair<RISCV64InstrInfo::BranchType, MachineInstr *>
...
char RISCV64DelaySlotFiller::ID = 0;

static bool hasUnoccupiedSlot(const MachineInstr *MI) {
  return MI->hasDelaySlot() && !MI->isBundledWithSucc();
}

INITIALIZE_PASS(RISCV64DelaySlotFiller, DEBUG_TYPE,
...
void RegDefsUses::setCallerSaved(const MachineInstr &MI) {
  ...
  if (MI.definesRegister(RISCV64::RA) || MI.definesRegister(RISCV64::RA_64)) {
    Defs.set(RISCV64::RA);
    Defs.set(RISCV64::RA_64);
  }

  // If MI is a call, add all caller-saved registers to Defs.
  BitVector CallerSavedRegs(TRI.getNumRegs(), true);

  CallerSavedRegs.reset(RISCV64::ZERO);
  CallerSavedRegs.reset(RISCV64::ZERO_64);
...
void RegDefsUses::setUnallocatableRegs(const MachineFunction &MF) {
  ...
  AllocSet.set(RISCV64::ZERO);
  AllocSet.set(RISCV64::ZERO_64);
...
Iter RISCV64DelaySlotFiller::replaceWithCompactBranch(MachineBasicBlock &MBB,
                                                   Iter Branch,
                                                   const DebugLoc &DL) {
  const RISCV64Subtarget &STI = MBB.getParent()->getSubtarget<RISCV64Subtarget>();
  const RISCV64InstrInfo *TII = STI.getInstrInfo();
...
static int getEquivalentCallShort(int Opcode) {
  switch (Opcode) {
  case RISCV64::BGEZAL:
    return RISCV64::BGEZALS_MM;
  case RISCV64::BLTZAL:
    return RISCV64::BLTZALS_MM;
  case RISCV64::JAL:
  case RISCV64::JAL_MM:
    return RISCV64::JALS_MM;
  case RISCV64::JALR:
    return RISCV64::JALRS_MM;
  case RISCV64::JALR16_MM:
    return RISCV64::JALRS16_MM;
  case RISCV64::TAILCALL_MM:
    llvm_unreachable("Attempting to shorten the TAILCALL_MM pseudo!");
  case RISCV64::TAILCALLREG:
    return RISCV64::JR16_MM;
  default:
...
bool RISCV64DelaySlotFiller::runOnMachineBasicBlock(MachineBasicBlock &MBB) {
  bool Changed = false;
  const RISCV64Subtarget &STI = MBB.getParent()->getSubtarget<RISCV64Subtarget>();
  bool InMicroRISCV64Mode = STI.inMicroRISCV64Mode();
  const RISCV64InstrInfo *TII = STI.getInstrInfo();
  ...
  for (Iter I = MBB.begin(); I != MBB.end(); ++I) {
    ...
    if (!DisableDelaySlotFiller && (TM->getOptLevel() != CodeGenOpt::None) &&
        !(InMicroRISCV64Mode && STI.hasRISCV6432r6())) {

      bool Filled = false;

      if (RISCV64CompactBranchPolicy.getValue() != CB_Always ||
      ...
      if (Filled) {
        ...
        if (InMicroRISCV64Mode && TII->getInstSizeInBytes(*std::next(DSI)) == 2 &&
            DSI->isCall()) {
    ...
    if ((InMicroRISCV64Mode ||
         (STI.hasRISCV6432r6() && RISCV64CompactBranchPolicy != CB_Never)) &&
        TII->getEquivalentCompactForm(I)) {
    ...
    BuildMI(MBB, std::next(I), I->getDebugLoc(), TII->get(RISCV64::NOP));
...
template <typename IterTy>
bool RISCV64DelaySlotFiller::searchRange(MachineBasicBlock &MBB, IterTy Begin,
  ...
  for (IterTy I = Begin; I != End;) {
    ...
    if (STI.isTargetNaCl()) {
      ...
      if ((isBasePlusOffsetMemoryAccess(CurrI->getOpcode(), &AddrIdx) &&
          ...
          CurrI->modifiesRegister(RISCV64::SP, STI.getRegisterInfo()))
        continue;
    }
    ...
    // but we don't have enough information to make that decision.
     if (InMicroRISCV64Mode && TII->getInstSizeInBytes(*CurrI) == 2 &&
        (Opcode == RISCV64::JR || Opcode == RISCV64::PseudoIndirectBranch ||
         Opcode == RISCV64::PseudoReturn || Opcode == RISCV64::TAILCALL))
      continue;
     // Instructions LWP/SWP and MOVEP should not be in a delay slot as that
     // results in unpredictable behaviour
     if (InMicroRISCV64Mode && (Opcode == RISCV64::LWP_MM || Opcode == RISCV64::SWP_MM ||
                             Opcode == RISCV64::MOVEP_MM))
       continue;
...
bool RISCV64DelaySlotFiller::searchBackward(MachineBasicBlock &MBB,
...
bool RISCV64DelaySlotFiller::searchForward(MachineBasicBlock &MBB,
                                        Iter Slot) const {
...
bool RISCV64DelaySlotFiller::searchSuccBBs(MachineBasicBlock &MBB,
                                        Iter Slot) const {
...
MachineBasicBlock *
RISCV64DelaySlotFiller::selectSuccBB(MachineBasicBlock &B) const {
  if (B.succ_empty())
    return nullptr;
...
std::pair<RISCV64InstrInfo::BranchType, MachineInstr *>
RISCV64DelaySlotFiller::getBranch(MachineBasicBlock &MBB,
                               const MachineBasicBlock &Dst) const {
  const RISCV64InstrInfo *TII =
      MBB.getParent()->getSubtarget<RISCV64Subtarget>().getInstrInfo();
  MachineBasicBlock *TrueBB = nullptr, *FalseBB = nullptr;
  SmallVector<MachineInstr*, 2> BranchInstrs;
  SmallVector<MachineOperand, 2> Cond;

  RISCV64InstrInfo::BranchType R =
      TII->analyzeBranch(MBB, TrueBB, FalseBB, Cond, false, BranchInstrs);

  if ((R == RISCV64InstrInfo::BT_None) || (R == RISCV64InstrInfo::BT_NoBranch))
    return std::make_pair(R, nullptr);

  if (R != RISCV64InstrInfo::BT_CondUncond) {
    if (!hasUnoccupiedSlot(BranchInstrs[0]))
      return std::make_pair(RISCV64InstrInfo::BT_None, nullptr);

    assert(((R != RISCV64InstrInfo::BT_Uncond) || (TrueBB == &Dst)));
  ...
  if (hasUnoccupiedSlot(BranchInstrs[0]))
    return std::make_pair(RISCV64InstrInfo::BT_Cond, BranchInstrs[0]);

  // If that fails, try the unconditional branch.
  if (hasUnoccupiedSlot(BranchInstrs[1]) && (FalseBB == &Dst))
    return std::make_pair(RISCV64InstrInfo::BT_Uncond, BranchInstrs[1]);

  return std::make_pair(RISCV64InstrInfo::BT_None, nullptr);
}

bool RISCV64DelaySlotFiller::examinePred(MachineBasicBlock &Pred,
                                      const MachineBasicBlock &Succ,
                                      RegDefsUses &RegDU,
                                      bool &HasMultipleSuccs,
                                      BB2BrMap &BrMap) const {
  std::pair<RISCV64InstrInfo::BranchType, MachineInstr *> P =
      getBranch(Pred, Succ);

  // Return if either getBranch wasn't able to analyze the branches or there
  // were no branches with unoccupied slots.
  if (P.first == RISCV64InstrInfo::BT_None)
    return false;

  if ((P.first != RISCV64InstrInfo::BT_Uncond) &&
      (P.first != RISCV64InstrInfo::BT_NoBranch)) {
...
bool RISCV64DelaySlotFiller::delayHasHazard(const MachineInstr &Candidate,
                                         RegDefsUses &RegDU,
                                         InspectMemInstr &IM) const {
...
bool RISCV64DelaySlotFiller::terminateSearch(const MachineInstr &Candidate) const {
  return (Candidate.isTerminator() || Candidate.isCall() ||
          Candidate.isPosition() || Candidate.isInlineAsm() ||
          Candidate.hasUnmodeledSideEffects());
}

/// createRISCV64DelaySlotFillerPass - Returns a pass that fills in delay
/// slots in RISCV64 MachineFunctions
FunctionPass *llvm::createRISCV64DelaySlotFillerPass() { return new RISCV64DelaySlotFiller(); }
