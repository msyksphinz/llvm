//===---- RISCV64Os16.cpp for RISCV64 Option -Os16                       --------===//
//
...
static cl::opt<std::string> RISCV6432FunctionMask(
...
namespace {
  class RISCV64Os16 : public ModulePass {
  public:
    static char ID;

    RISCV64Os16() : ModulePass(ID) {}
...
bool RISCV64Os16::runOnModule(Module &M) {
  bool usingMask = RISCV6432FunctionMask.length() > 0;
  bool doneUsingMask = false; // this will make it stop repeating

  LLVM_DEBUG(dbgs() << "Run on Module RISCV64Os16 \n"
                    << RISCV6432FunctionMask << "\n");
  if (usingMask)
  ...
  for (auto &F : M) {
    ...
    if (usingMask) {
      if (!doneUsingMask) {
        if (functionIndex == RISCV6432FunctionMask.length())
          functionIndex = 0;
        switch (RISCV6432FunctionMask[functionIndex]) {
        case '1':
...
ModulePass *llvm::createRISCV64Os16Pass() { return new RISCV64Os16(); }
