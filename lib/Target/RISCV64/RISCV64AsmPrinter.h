//===- RISCV64AsmPrinter.h - RISCV64 LLVM Assembly Printer -----------*- C++ -*--===//
//
...
//===----------------------------------------------------------------------===//
//
// RISCV64 Assembly printer class.
//
//===----------------------------------------------------------------------===//
...
class RISCV64TargetStreamer;
class Module;
class raw_ostream;
class TargetMachine;

class LLVM_LIBRARY_VISIBILITY RISCV64AsmPrinter : public AsmPrinter {
  RISCV64TargetStreamer &getTargetStreamer() const;
...
public:
  // XRay-specific lowering for RISCV64.
...
private:
  ...
  std::map<const char *, const RISCV6416HardFloatInfo::FuncSignature *>
      StubsNeeded;
  ...
  void EmitSwapFPIntParams(const MCSubtargetInfo &STI,
                           RISCV6416HardFloatInfo::FPParamVariant, bool LE,
                           bool ToFP);

  void EmitSwapFPIntRetval(const MCSubtargetInfo &STI,
                           RISCV6416HardFloatInfo::FPReturnVariant, bool LE);

  void EmitFPCallStub(const char *, const RISCV6416HardFloatInfo::FuncSignature *);
...
public:
  ...
  const RISCV64FunctionInfo *RISCV64FI;
  RISCV64MCInstLower MCInstLowering;

  explicit RISCV64AsmPrinter(TargetMachine &TM,
                          std::unique_ptr<MCStreamer> Streamer)
      : AsmPrinter(TM, std::move(Streamer)), MCInstLowering(*this) {}

  StringRef getPassName() const override { return "RISCV64 Assembly Printer"; }

  bool runOnMachineFunction(MachineFunction &MF) override;

  void EmitConstantPool() override {
    bool UsingConstantPools =
      (Subtarget->inRISCV6416Mode() && Subtarget->useConstantIslands());
