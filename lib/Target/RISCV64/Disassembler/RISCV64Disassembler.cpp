//===- RISCV64Disassembler.cpp - Disassembler for RISCV64 -----------------------===//
//
...
//===----------------------------------------------------------------------===//
//
// This file is part of the RISCV64 Disassembler.
//
//===----------------------------------------------------------------------===//
...
class RISCV64Disassembler : public MCDisassembler {
  bool IsMicroRISCV64;
  bool IsBigEndian;

public:
  RISCV64Disassembler(const MCSubtargetInfo &STI, MCContext &Ctx, bool IsBigEndian)
      : MCDisassembler(STI, Ctx),
        IsMicroRISCV64(STI.getFeatureBits()[RISCV64::FeatureMicroRISCV64]),
        IsBigEndian(IsBigEndian) {}

  bool hasRISCV642() const { return STI.getFeatureBits()[RISCV64::FeatureRISCV642]; }
  bool hasRISCV643() const { return STI.getFeatureBits()[RISCV64::FeatureRISCV643]; }
  bool hasRISCV6432() const { return STI.getFeatureBits()[RISCV64::FeatureRISCV6432]; }

  bool hasRISCV6432r6() const {
    return STI.getFeatureBits()[RISCV64::FeatureRISCV6432r6];
  }

  bool isFP64() const { return STI.getFeatureBits()[RISCV64::FeatureFP64Bit]; }

  bool isGP64() const { return STI.getFeatureBits()[RISCV64::FeatureGP64Bit]; }

  bool isPTR64() const { return STI.getFeatureBits()[RISCV64::FeaturePTR64Bit]; }

  bool hasCnRISCV64() const { return STI.getFeatureBits()[RISCV64::FeatureCnRISCV64]; }

  bool hasCOP3() const {
    // Only present in MIPS-I and MIPS-II
    return !hasRISCV6432() && !hasRISCV643();
  }
...
static MCDisassembler *createRISCV64Disassembler(
                       const Target &T,
                       const MCSubtargetInfo &STI,
                       MCContext &Ctx) {
  return new RISCV64Disassembler(STI, Ctx, true);
}

static MCDisassembler *createRISCV64elDisassembler(
                       const Target &T,
                       const MCSubtargetInfo &STI,
                       MCContext &Ctx) {
  return new RISCV64Disassembler(STI, Ctx, false);
}

extern "C" void LLVMInitializeRISCV64Disassembler() {
  // Register the disassembler.
  TargetRegistry::RegisterMCDisassembler(getTheRISCV64Target(),
                                         createRISCV64Disassembler);
  TargetRegistry::RegisterMCDisassembler(getTheRISCV64elTarget(),
                                         createRISCV64elDisassembler);
  TargetRegistry::RegisterMCDisassembler(getTheRISCV6464Target(),
                                         createRISCV64Disassembler);
  TargetRegistry::RegisterMCDisassembler(getTheRISCV6464elTarget(),
                                         createRISCV64elDisassembler);
}

#include "RISCV64GenDisassemblerTables.inc"

static unsigned getReg(const void *D, unsigned RC, unsigned RegNo) {
  const RISCV64Disassembler *Dis = static_cast<const RISCV64Disassembler*>(D);
...
static DecodeStatus DecodeDAHIDATIMMR6(MCInst &MI, InsnType insn, uint64_t Address,
  ...
  MI.addOperand(MCOperand::createReg(getReg(Decoder, RISCV64::GPR64RegClassID,
                                       Rs)));
  MI.addOperand(MCOperand::createReg(getReg(Decoder, RISCV64::GPR64RegClassID,
                                       Rs)));
...
static DecodeStatus DecodeAddiGroupBranch(MCInst &MI, InsnType insn,
  ...
  if (Rs >= Rt) {
    MI.setOpcode(RISCV64::BOVC);
    HasRs = true;
  } else if (Rs != 0 && Rs < Rt) {
    MI.setOpcode(RISCV64::BEQC);
    HasRs = true;
  } else
    MI.setOpcode(RISCV64::BEQZALC);

  if (HasRs)
    MI.addOperand(MCOperand::createReg(getReg(Decoder, RISCV64::GPR32RegClassID,
                                       Rs)));
...
static DecodeStatus DecodePOP35GroupBranchMMR6(MCInst &MI, InsnType insn,
  ...
  if (Rs >= Rt) {
    MI.setOpcode(RISCV64::BOVC_MMR6);
    MI.addOperand(MCOperand::createReg(getReg(Decoder, RISCV64::GPR32RegClassID,
                                       Rt)));
    MI.addOperand(MCOperand::createReg(getReg(Decoder, RISCV64::GPR32RegClassID,
                                       Rs)));
    ...
    MI.setOpcode(RISCV64::BEQC_MMR6);
    MI.addOperand(MCOperand::createReg(getReg(Decoder, RISCV64::GPR32RegClassID,
                                       Rs)));
    MI.addOperand(MCOperand::createReg(getReg(Decoder, RISCV64::GPR32RegClassID,
                                       Rt)));
    ...
    MI.setOpcode(RISCV64::BEQZALC_MMR6);
    MI.addOperand(MCOperand::createReg(getReg(Decoder, RISCV64::GPR32RegClassID,
                                       Rt)));
    Imm = SignExtend64(fieldFromInstruction(insn, 0, 16), 16) * 2 + 4;
  }
...
static DecodeStatus DecodeDaddiGroupBranch(MCInst &MI, InsnType insn,
  ...
  if (Rs >= Rt) {
    MI.setOpcode(RISCV64::BNVC);
    HasRs = true;
  } else if (Rs != 0 && Rs < Rt) {
    MI.setOpcode(RISCV64::BNEC);
    HasRs = true;
  } else
    MI.setOpcode(RISCV64::BNEZALC);
...
static DecodeStatus DecodePOP37GroupBranchMMR6(MCInst &MI, InsnType insn,
  ...
  if (Rs >= Rt) {
    MI.setOpcode(RISCV64::BNVC_MMR6);
    MI.addOperand(MCOperand::createReg(getReg(Decoder, RISCV64::GPR32RegClassID,
                                       Rt)));
    MI.addOperand(MCOperand::createReg(getReg(Decoder, RISCV64::GPR32RegClassID,
                                       Rs)));
    ...
    MI.setOpcode(RISCV64::BNEC_MMR6);
    MI.addOperand(MCOperand::createReg(getReg(Decoder, RISCV64::GPR32RegClassID,
                                       Rs)));
    MI.addOperand(MCOperand::createReg(getReg(Decoder, RISCV64::GPR32RegClassID,
                                       Rt)));
    ...
    MI.setOpcode(RISCV64::BNEZALC_MMR6);
    MI.addOperand(MCOperand::createReg(getReg(Decoder, RISCV64::GPR32RegClassID,
                                       Rt)));
    Imm = SignExtend64(fieldFromInstruction(insn, 0, 16), 16) * 2 + 4;
  }
...
static DecodeStatus DecodePOP65GroupBranchMMR6(MCInst &MI, InsnType insn,
  ...
  if (Rt == 0)
    ...
    MI.setOpcode(RISCV64::BGTZC_MMR6);
  else if (Rs == Rt)
    MI.setOpcode(RISCV64::BLTZC_MMR6);
  else {
    MI.setOpcode(RISCV64::BLTC_MMR6);
    HasRs = true;
  }
...
static DecodeStatus DecodePOP75GroupBranchMMR6(MCInst &MI, InsnType insn,
  ...
  if (Rt == 0)
    ...
    MI.setOpcode(RISCV64::BLEZC_MMR6);
  else if (Rs == Rt)
    MI.setOpcode(RISCV64::BGEZC_MMR6);
  else {
    HasRs = true;
    MI.setOpcode(RISCV64::BGEC_MMR6);
  }
...
static DecodeStatus DecodeBlezlGroupBranch(MCInst &MI, InsnType insn,
  ...
  if (Rt == 0)
    ...
    MI.setOpcode(RISCV64::BLEZC);
  else if (Rs == Rt)
    MI.setOpcode(RISCV64::BGEZC);
  else {
    HasRs = true;
    MI.setOpcode(RISCV64::BGEC);
  }
...
static DecodeStatus DecodeBgtzlGroupBranch(MCInst &MI, InsnType insn,
  ...
  if (Rt == 0)
    ...
    MI.setOpcode(RISCV64::BGTZC);
  else if (Rs == Rt)
    MI.setOpcode(RISCV64::BLTZC);
  else {
    MI.setOpcode(RISCV64::BLTC);
    HasRs = true;
  }
...
static DecodeStatus DecodeBgtzGroupBranch(MCInst &MI, InsnType insn,
  ...
  if (Rt == 0) {
    MI.setOpcode(RISCV64::BGTZ);
    HasRs = true;
  } else if (Rs == 0) {
    MI.setOpcode(RISCV64::BGTZALC);
    HasRt = true;
  } else if (Rs == Rt) {
    MI.setOpcode(RISCV64::BLTZALC);
    HasRs = true;
  } else {
    MI.setOpcode(RISCV64::BLTUC);
    HasRs = true;
    HasRt = true;
  }
...
static DecodeStatus DecodeBlezGroupBranch(MCInst &MI, InsnType insn,
  ...
  if (Rt == 0)
    ...
    MI.setOpcode(RISCV64::BLEZALC);
  else if (Rs == Rt)
    MI.setOpcode(RISCV64::BGEZALC);
  else {
    HasRs = true;
    MI.setOpcode(RISCV64::BGEUC);
  }
...
static DecodeStatus DecodeDEXT(MCInst &MI, InsnType Insn, uint64_t Address,
  ...
  MI.setOpcode(RISCV64::DEXT);

  InsnType Rs = fieldFromInstruction(Insn, 21, 5);
  InsnType Rt = fieldFromInstruction(Insn, 16, 5);

  MI.addOperand(MCOperand::createReg(getReg(Decoder, RISCV64::GPR64RegClassID, Rt)));
  MI.addOperand(MCOperand::createReg(getReg(Decoder, RISCV64::GPR64RegClassID, Rs)));
...
static DecodeStatus readInstruction32(ArrayRef<uint8_t> Bytes, uint64_t Address,
                                      uint64_t &Size, uint32_t &Insn,
                                      bool IsBigEndian, bool IsMicroRISCV64) {
...
DecodeStatus RISCV64Disassembler::getInstruction(MCInst &Instr, uint64_t &Size,
  ...
  if (IsMicroRISCV64) {
    ...
    if (hasRISCV6432r6()) {
      LLVM_DEBUG(
          dbgs() << "Trying MicroRISCV64R616 table (16-bit instructions):\n");
      // Calling the auto-generated decoder function for microMIPS32R6
      // 16-bit instructions.
      Result = decodeInstruction(DecoderTableMicroRISCV64R616, Instr, Insn,
                                 Address, this, STI);
      if (Result != MCDisassembler::Fail) {
        Size = 2;
        return Result;
    ...
    LLVM_DEBUG(dbgs() << "Trying MicroRISCV6416 table (16-bit instructions):\n");
    // Calling the auto-generated decoder function for microMIPS 16-bit
    // instructions.
    Result = decodeInstruction(DecoderTableMicroRISCV6416, Instr, Insn, Address,
                               this, STI);
    if (Result != MCDisassembler::Fail) {
      Size = 2;
      return Result;
    ...
    if (hasRISCV6432r6()) {
      LLVM_DEBUG(
          dbgs() << "Trying MicroRISCV6432r632 table (32-bit instructions):\n");
      // Calling the auto-generated decoder function.
      Result = decodeInstruction(DecoderTableMicroRISCV64R632, Instr, Insn, Address,
                                 this, STI);
      if (Result != MCDisassembler::Fail) {
        Size = 4;
        return Result;
    ...
    LLVM_DEBUG(dbgs() << "Trying MicroRISCV6432 table (32-bit instructions):\n");
    // Calling the auto-generated decoder function.
    Result = decodeInstruction(DecoderTableMicroRISCV6432, Instr, Insn, Address,
                               this, STI);
    if (Result != MCDisassembler::Fail) {
      Size = 4;
      return Result;
    ...
    if (isFP64()) {
      LLVM_DEBUG(dbgs() << "Trying MicroRISCV64FP64 table (32-bit opcodes):\n");
      Result = decodeInstruction(DecoderTableMicroRISCV64FP6432, Instr, Insn,
                                 Address, this, STI);
      if (Result != MCDisassembler::Fail) {
        Size = 4;
        return Result;
  ...
  if (hasRISCV6432r6() && isGP64()) {
    LLVM_DEBUG(
        dbgs() << "Trying RISCV6432r6_64r6 (GPR64) table (32-bit opcodes):\n");
    Result = decodeInstruction(DecoderTableRISCV6432r6_64r6_GP6432, Instr, Insn,
                               Address, this, STI);
    if (Result != MCDisassembler::Fail)
      return Result;
  }

  if (hasRISCV6432r6() && isPTR64()) {
    LLVM_DEBUG(
        dbgs() << "Trying RISCV6432r6_64r6 (PTR64) table (32-bit opcodes):\n");
    Result = decodeInstruction(DecoderTableRISCV6432r6_64r6_PTR6432, Instr, Insn,
                               Address, this, STI);
    if (Result != MCDisassembler::Fail)
      return Result;
  }
  ...
  if (hasRISCV6432r6()) {
    LLVM_DEBUG(dbgs() << "Trying RISCV6432r6_64r6 table (32-bit opcodes):\n");
    Result = decodeInstruction(DecoderTableRISCV6432r6_64r632, Instr, Insn,
                               Address, this, STI);
    if (Result != MCDisassembler::Fail)
      return Result;
  }

  if (hasRISCV642() && isPTR64()) {
    LLVM_DEBUG(
        dbgs() << "Trying RISCV6432r6_64r6 (PTR64) table (32-bit opcodes):\n");
    Result = decodeInstruction(DecoderTableRISCV6432_64_PTR6432, Instr, Insn,
                               Address, this, STI);
    if (Result != MCDisassembler::Fail)
      return Result;
  }
  ...
  if (hasCnRISCV64()) {
    LLVM_DEBUG(dbgs() << "Trying CnRISCV64 table (32-bit opcodes):\n");
    Result = decodeInstruction(DecoderTableCnRISCV6432, Instr, Insn,
                               Address, this, STI);
    if (Result != MCDisassembler::Fail)
      return Result;
  }
  ...
  if (isGP64()) {
    LLVM_DEBUG(dbgs() << "Trying RISCV6464 (GPR64) table (32-bit opcodes):\n");
    Result = decodeInstruction(DecoderTableRISCV646432, Instr, Insn,
                               Address, this, STI);
    if (Result != MCDisassembler::Fail)
      return Result;
  }
  ...
  if (isFP64()) {
    LLVM_DEBUG(
        dbgs() << "Trying RISCV64FP64 (64 bit FPU) table (32-bit opcodes):\n");
    Result = decodeInstruction(DecoderTableRISCV64FP6432, Instr, Insn,
                               Address, this, STI);
    if (Result != MCDisassembler::Fail)
      return Result;
  }

  LLVM_DEBUG(dbgs() << "Trying RISCV64 table (32-bit opcodes):\n");
  // Calling the auto-generated decoder function.
  Result =
      decodeInstruction(DecoderTableRISCV6432, Instr, Insn, Address, this, STI);
  if (Result != MCDisassembler::Fail)
    return Result;
...
static DecodeStatus DecodeGPR64RegisterClass(MCInst &Inst,
  ...
  unsigned Reg = getReg(Decoder, RISCV64::GPR64RegClassID, RegNo);
...
static DecodeStatus DecodeGPRMM16RegisterClass(MCInst &Inst,
  ...
  if (RegNo > 7)
    return MCDisassembler::Fail;
  unsigned Reg = getReg(Decoder, RISCV64::GPRMM16RegClassID, RegNo);
  Inst.addOperand(MCOperand::createReg(Reg));
  return MCDisassembler::Success;
}
...
static DecodeStatus DecodeGPRMM16ZeroRegisterClass(MCInst &Inst,
  ...
  if (RegNo > 7)
    return MCDisassembler::Fail;
  unsigned Reg = getReg(Decoder, RISCV64::GPRMM16ZeroRegClassID, RegNo);
  Inst.addOperand(MCOperand::createReg(Reg));
  return MCDisassembler::Success;
}
...
static DecodeStatus DecodeGPRMM16MovePRegisterClass(MCInst &Inst,
  ...
  if (RegNo > 7)
    return MCDisassembler::Fail;
  unsigned Reg = getReg(Decoder, RISCV64::GPRMM16MovePRegClassID, RegNo);
  Inst.addOperand(MCOperand::createReg(Reg));
  return MCDisassembler::Success;
}
...
static DecodeStatus DecodeGPR32RegisterClass(MCInst &Inst,
  ...
  if (RegNo > 31)
    return MCDisassembler::Fail;
  unsigned Reg = getReg(Decoder, RISCV64::GPR32RegClassID, RegNo);
  Inst.addOperand(MCOperand::createReg(Reg));
  return MCDisassembler::Success;
}
...
static DecodeStatus DecodePtrRegisterClass(MCInst &Inst,
                                           ...
                                           uint64_t Address,
                                           const void *Decoder) {
  if (static_cast<const RISCV64Disassembler *>(Decoder)->isGP64())
...
static DecodeStatus DecodeFGR64RegisterClass(MCInst &Inst,
  ...
  unsigned Reg = getReg(Decoder, RISCV64::FGR64RegClassID, RegNo);
  Inst.addOperand(MCOperand::createReg(Reg));
  return MCDisassembler::Success;
}
...
static DecodeStatus DecodeFGR32RegisterClass(MCInst &Inst,
  ...
  unsigned Reg = getReg(Decoder, RISCV64::FGR32RegClassID, RegNo);
  Inst.addOperand(MCOperand::createReg(Reg));
  return MCDisassembler::Success;
}
...
static DecodeStatus DecodeCCRRegisterClass(MCInst &Inst,
  ...
  if (RegNo > 31)
    return MCDisassembler::Fail;
  unsigned Reg = getReg(Decoder, RISCV64::CCRRegClassID, RegNo);
  Inst.addOperand(MCOperand::createReg(Reg));
  return MCDisassembler::Success;
}
...
static DecodeStatus DecodeFCCRegisterClass(MCInst &Inst,
  ...
  if (RegNo > 7)
    return MCDisassembler::Fail;
  unsigned Reg = getReg(Decoder, RISCV64::FCCRegClassID, RegNo);
  Inst.addOperand(MCOperand::createReg(Reg));
  return MCDisassembler::Success;
}
...
static DecodeStatus DecodeFGRCCRegisterClass(MCInst &Inst, unsigned RegNo,
  ...
  unsigned Reg = getReg(Decoder, RISCV64::FGRCCRegClassID, RegNo);
  Inst.addOperand(MCOperand::createReg(Reg));
  return MCDisassembler::Success;
}
...
static DecodeStatus DecodeMem(MCInst &Inst,
  ...
  Reg = getReg(Decoder, RISCV64::GPR32RegClassID, Reg);
  Base = getReg(Decoder, RISCV64::GPR32RegClassID, Base);

  if (Inst.getOpcode() == RISCV64::SC ||
      Inst.getOpcode() == RISCV64::SCD)
    Inst.addOperand(MCOperand::createReg(Reg));
...
static DecodeStatus DecodeMSA128Mem(MCInst &Inst, unsigned Insn,
  ...
  Reg = getReg(Decoder, RISCV64::MSA128BRegClassID, Reg);
  Base = getReg(Decoder, RISCV64::GPR32RegClassID, Base);
...
static DecodeStatus DecodeMemMMImm9(MCInst &Inst,
  ...
  if (Inst.getOpcode() == RISCV64::SCE_MM || Inst.getOpcode() == RISCV64::SC_MMR6)
    Inst.addOperand(MCOperand::createReg(Reg));
...
static DecodeStatus DecodeMemMMImm12(MCInst &Inst,
  ...
  default:
    Inst.addOperand(MCOperand::createReg(Reg));
    if (Inst.getOpcode() == RISCV64::LWP_MM || Inst.getOpcode() == RISCV64::SWP_MM)
...
static DecodeStatus DecodeFMem(MCInst &Inst,
  ...
  Reg = getReg(Decoder, RISCV64::FGR64RegClassID, Reg);
  Base = getReg(Decoder, RISCV64::GPR32RegClassID, Base);
...
static DecodeStatus DecodeFMem2(MCInst &Inst,
  ...
  Reg = getReg(Decoder, RISCV64::COP2RegClassID, Reg);
  Base = getReg(Decoder, RISCV64::GPR32RegClassID, Base);
...
static DecodeStatus DecodeFMem3(MCInst &Inst,
  ...
  Reg = getReg(Decoder, RISCV64::COP3RegClassID, Reg);
  Base = getReg(Decoder, RISCV64::GPR32RegClassID, Base);
...
static DecodeStatus DecodeSpecial3LlSc(MCInst &Inst,
  ...
  Rt = getReg(Decoder, RISCV64::GPR32RegClassID, Rt);
  Base = getReg(Decoder, RISCV64::GPR32RegClassID, Base);

  if(Inst.getOpcode() == RISCV64::SC_R6 || Inst.getOpcode() == RISCV64::SCD_R6){
...
static DecodeStatus DecodeHWRegsRegisterClass(MCInst &Inst,
  ...
  if (RegNo != 29)
    return  MCDisassembler::Fail;
  Inst.addOperand(MCOperand::createReg(RISCV64::HWR29));
  return MCDisassembler::Success;
}
...
static DecodeStatus DecodeAFGR64RegisterClass(MCInst &Inst,
  ...
  unsigned Reg = getReg(Decoder, RISCV64::AFGR64RegClassID, RegNo /2);
  Inst.addOperand(MCOperand::createReg(Reg));
  return MCDisassembler::Success;
}
...
static DecodeStatus DecodeACC64DSPRegisterClass(MCInst &Inst,
  ...
  unsigned Reg = getReg(Decoder, RISCV64::ACC64DSPRegClassID, RegNo);
  Inst.addOperand(MCOperand::createReg(Reg));
  return MCDisassembler::Success;
}
...
static DecodeStatus DecodeHI32DSPRegisterClass(MCInst &Inst,
  ...
  unsigned Reg = getReg(Decoder, RISCV64::HI32DSPRegClassID, RegNo);
  Inst.addOperand(MCOperand::createReg(Reg));
  return MCDisassembler::Success;
}
...
static DecodeStatus DecodeLO32DSPRegisterClass(MCInst &Inst,
  ...
  unsigned Reg = getReg(Decoder, RISCV64::LO32DSPRegClassID, RegNo);
  Inst.addOperand(MCOperand::createReg(Reg));
  return MCDisassembler::Success;
}
...
static DecodeStatus DecodeMSA128BRegisterClass(MCInst &Inst,
  ...
  unsigned Reg = getReg(Decoder, RISCV64::MSA128BRegClassID, RegNo);
  Inst.addOperand(MCOperand::createReg(Reg));
  return MCDisassembler::Success;
}
...
static DecodeStatus DecodeMSA128HRegisterClass(MCInst &Inst,
  ...
  unsigned Reg = getReg(Decoder, RISCV64::MSA128HRegClassID, RegNo);
  Inst.addOperand(MCOperand::createReg(Reg));
  return MCDisassembler::Success;
}
...
static DecodeStatus DecodeMSA128WRegisterClass(MCInst &Inst,
  ...
  unsigned Reg = getReg(Decoder, RISCV64::MSA128WRegClassID, RegNo);
  Inst.addOperand(MCOperand::createReg(Reg));
  return MCDisassembler::Success;
}
...
static DecodeStatus DecodeMSA128DRegisterClass(MCInst &Inst,
  ...
  unsigned Reg = getReg(Decoder, RISCV64::MSA128DRegClassID, RegNo);
  Inst.addOperand(MCOperand::createReg(Reg));
  return MCDisassembler::Success;
}
...
static DecodeStatus DecodeMSACtrlRegisterClass(MCInst &Inst,
  ...
  unsigned Reg = getReg(Decoder, RISCV64::MSACtrlRegClassID, RegNo);
  Inst.addOperand(MCOperand::createReg(Reg));
  return MCDisassembler::Success;
}
...
static DecodeStatus DecodeCOP0RegisterClass(MCInst &Inst,
  ...
  unsigned Reg = getReg(Decoder, RISCV64::COP0RegClassID, RegNo);
  Inst.addOperand(MCOperand::createReg(Reg));
  return MCDisassembler::Success;
}
...
static DecodeStatus DecodeCOP2RegisterClass(MCInst &Inst,
  ...
  unsigned Reg = getReg(Decoder, RISCV64::COP2RegClassID, RegNo);
  Inst.addOperand(MCOperand::createReg(Reg));
  return MCDisassembler::Success;
}
...
static DecodeStatus DecodeRegListOperand(MCInst &Inst,
                                         ...
                                         uint64_t Address,
                                         const void *Decoder) {
  unsigned Regs[] = {RISCV64::S0, RISCV64::S1, RISCV64::S2, RISCV64::S3, RISCV64::S4, RISCV64::S5,
                     RISCV64::S6, RISCV64::S7, RISCV64::FP};
  unsigned RegNum;
  ...
  if (RegLst & 0x10)
    Inst.addOperand(MCOperand::createReg(RISCV64::RA));
...
static DecodeStatus DecodeRegListOperand16(MCInst &Inst, unsigned Insn,
                                           uint64_t Address,
                                           const void *Decoder) {
  unsigned Regs[] = {RISCV64::S0, RISCV64::S1, RISCV64::S2, RISCV64::S3};
  unsigned RegLst;
  switch(Inst.getOpcode()) {
  default:
...
static DecodeStatus DecodeMovePOperands(MCInst &Inst, unsigned Insn,
  ...
  unsigned RegRs;
  if (static_cast<const RISCV64Disassembler*>(Decoder)->hasRISCV6432r6())
...
static DecodeStatus DecodeMovePRegPair(MCInst &Inst, unsigned RegPair,
  ...
  default:
    return MCDisassembler::Fail;
  case 0:
    Inst.addOperand(MCOperand::createReg(RISCV64::A1));
    Inst.addOperand(MCOperand::createReg(RISCV64::A2));
    break;
  case 1:
    Inst.addOperand(MCOperand::createReg(RISCV64::A1));
    Inst.addOperand(MCOperand::createReg(RISCV64::A3));
    break;
  case 2:
    Inst.addOperand(MCOperand::createReg(RISCV64::A2));
    Inst.addOperand(MCOperand::createReg(RISCV64::A3));
    break;
  case 3:
    Inst.addOperand(MCOperand::createReg(RISCV64::A0));
    Inst.addOperand(MCOperand::createReg(RISCV64::S5));
    break;
  case 4:
    Inst.addOperand(MCOperand::createReg(RISCV64::A0));
    Inst.addOperand(MCOperand::createReg(RISCV64::S6));
    break;
  case 5:
    Inst.addOperand(MCOperand::createReg(RISCV64::A0));
    Inst.addOperand(MCOperand::createReg(RISCV64::A1));
...
static DecodeStatus DecodeBgtzGroupBranchMMR6(MCInst &MI, InsnType insn,
  ...
  if (Rt == 0)
    ...
    MI.setOpcode(RISCV64::BGTZALC_MMR6);
    HasRt = true;
    Imm = SignExtend64(fieldFromInstruction(insn, 0, 16), 16) * 2 + 4;
  }
  else if (Rs == Rt) {
    MI.setOpcode(RISCV64::BLTZALC_MMR6);
    HasRs = true;
    Imm = SignExtend64(fieldFromInstruction(insn, 0, 16), 16) * 2 + 4;
  }
  else {
    MI.setOpcode(RISCV64::BLTUC_MMR6);
    HasRs = true;
    HasRt = true;
    Imm = SignExtend64(fieldFromInstruction(insn, 0, 16), 16) * 4 + 4;
  }
  ...
  if (HasRs)
    MI.addOperand(
    MCOperand::createReg(getReg(Decoder, RISCV64::GPR32RegClassID, Rs)));

  if (HasRt)
    MI.addOperand(
    MCOperand::createReg(getReg(Decoder, RISCV64::GPR32RegClassID, Rt)));
...
static DecodeStatus DecodeBlezGroupBranchMMR6(MCInst &MI, InsnType insn,
  ...
  if (Rt == 0)
    ...
    MI.setOpcode(RISCV64::BLEZALC_MMR6);
    Imm = SignExtend64(fieldFromInstruction(insn, 0, 16), 16) * 2 + 4;
  }
  else if (Rs == Rt) {
    MI.setOpcode(RISCV64::BGEZALC_MMR6);
    Imm = SignExtend64(fieldFromInstruction(insn, 0, 16), 16) * 2 + 4;
  }
  else {
    HasRs = true;
    MI.setOpcode(RISCV64::BGEUC_MMR6);
    Imm = SignExtend64(fieldFromInstruction(insn, 0, 16), 16) * 4 + 4;
  }
