//===- RISCV64ConstantIslandPass.cpp - Emit Pc Relative loads ----------------===//
//
...
// For now, only RISCV6416 will use this.
//
// Loading constants inline is expensive on RISCV6416 and it's in general better
...
static unsigned int branchTargetOperand(MachineInstr *MI) {
  switch (MI->getOpcode()) {
  case RISCV64::Bimm16:
  case RISCV64::BimmX16:
  case RISCV64::Bteqz16:
  case RISCV64::BteqzX16:
  case RISCV64::Btnez16:
  case RISCV64::BtnezX16:
  case RISCV64::JalB16:
    return 0;
  case RISCV64::BeqzRxImm16:
  case RISCV64::BeqzRxImmX16:
  case RISCV64::BnezRxImm16:
  case RISCV64::BnezRxImmX16:
    return 1;
  }
...
static unsigned int longformBranchOpcode(unsigned int Opcode) {
  ...
  case RISCV64::Bimm16:
  case RISCV64::BimmX16:
    return RISCV64::BimmX16;
  case RISCV64::Bteqz16:
  case RISCV64::BteqzX16:
    return RISCV64::BteqzX16;
  case RISCV64::Btnez16:
  case RISCV64::BtnezX16:
    return RISCV64::BtnezX16;
  case RISCV64::JalB16:
    return RISCV64::JalB16;
  case RISCV64::BeqzRxImm16:
  case RISCV64::BeqzRxImmX16:
    return RISCV64::BeqzRxImmX16;
  case RISCV64::BnezRxImm16:
  case RISCV64::BnezRxImmX16:
    return RISCV64::BnezRxImmX16;
  }
  llvm_unreachable("Unknown branch type");
}
...
namespace {
  ...
  /// RISCV64ConstantIslands - Due to limited PC-relative displacements, RISCV64
  ...
  class RISCV64ConstantIslands : public MachineFunctionPass {
  ...
  const RISCV64Subtarget *STI = nullptr;
  const RISCV6416InstrInfo *TII;
  RISCV64FunctionInfo *MFI;
  ...
  public:
    ...
    RISCV64ConstantIslands() : MachineFunctionPass(ID) {}

    StringRef getPassName() const override { return "RISCV64 Constant Islands"; }
...
char RISCV64ConstantIslands::ID = 0;

bool RISCV64ConstantIslands::isOffsetInRange
...
LLVM_DUMP_METHOD void RISCV64ConstantIslands::dumpBBs() {
...
bool RISCV64ConstantIslands::runOnMachineFunction(MachineFunction &mf) {
  ...
  LLVM_DEBUG(dbgs() << "constant island machine function "
                    << "\n");
  if (!STI->inRISCV6416Mode() || !RISCV64Subtarget::useConstantIslands()) {
    return false;
  }
  TII = (const RISCV6416InstrInfo *)STI->getInstrInfo();
  MFI = MF->getInfo<RISCV64FunctionInfo>();
...
void
RISCV64ConstantIslands::doInitialPlacement(std::vector<MachineInstr*> &CPEMIs) {
  ...
  for (unsigned i = 0, e = CPs.size(); i != e; ++i) {
    ...
    MachineInstr *CPEMI =
      BuildMI(*BB, InsAt, DebugLoc(), TII->get(RISCV64::CONSTPOOL_ENTRY))
...
RISCV64ConstantIslands::CPEntry
*RISCV64ConstantIslands::findConstPoolEntry(unsigned CPI,
...
unsigned RISCV64ConstantIslands::getCPELogAlign(const MachineInstr &CPEMI) {
  assert(CPEMI.getOpcode() == RISCV64::CONSTPOOL_ENTRY);
    ...
    for (MachineInstr &MI : MBB) {
      ...
      for (unsigned op = 0, e = MI.getNumOperands(); op != e; ++op)
        if (MI.getOperand(op).isCPI()) {
          ...
          case RISCV64::LwRxPcTcp16:
            Bits = 8;
            Scale = 4;
            LongFormOpcode = RISCV64::LwRxPcTcpX16;
            LongFormBits = 14;
            LongFormScale = 1;
            break;
          case RISCV64::LwRxPcTcpX16:
            Bits = 14;
            Scale = 1;
            NegOk = true;
            break;
...
void RISCV64ConstantIslands::computeBlockSize(MachineBasicBlock *MBB) {
...
unsigned RISCV64ConstantIslands::getOffsetOf(MachineInstr *MI) const {
...
void RISCV64ConstantIslands::updateForInsertedWaterBlock
...
unsigned RISCV64ConstantIslands::getUserOffset(CPUser &U) const {
...
MachineBasicBlock *
RISCV64ConstantIslands::splitBlockBeforeInstr(MachineInstr &MI) {
  ...
  BuildMI(OrigBB, DebugLoc(), TII->get(RISCV64::Bimm16)).addMBB(NewBB);
  ++NumSplit;
...
/// constant pool entry).
bool RISCV64ConstantIslands::isOffsetInRange(unsigned UserOffset,
...
bool RISCV64ConstantIslands::isWaterInRange(unsigned UserOffset,
...
bool RISCV64ConstantIslands::isCPEntryInRange
...
static bool BBIsJumpedOver(MachineBasicBlock *MBB) {
  ...
  if (PredMI->getOpcode() == RISCV64::Bimm16)
    return PredMI->getOperand(0).getMBB() == Succ;
  return false;
}
#endif

void RISCV64ConstantIslands::adjustBBOffsetsAfter(MachineBasicBlock *BB) {
...
bool RISCV64ConstantIslands::decrementCPEReferenceCount(unsigned CPI,
                                                    MachineInstr *CPEMI) {
...
int RISCV64ConstantIslands::findInRangeCPEntry(CPUser& U, unsigned UserOffset)
{
...
/// 1 = entry found, and there were no code insertions or deletions
/// 2 = entry found, and there were code insertions or deletions
int RISCV64ConstantIslands::findLongFormInRangeCPEntry
...
bool RISCV64ConstantIslands::findAvailableWater(CPUser &U, unsigned UserOffset,
                                      water_iterator &WaterIter) {
  if (WaterList.empty())
    return false;
...
/// is not adjusted).
void RISCV64ConstantIslands::createNewWater(unsigned CPUserIndex,
                                        unsigned UserOffset,
                                        MachineBasicBlock *&NewMBB) {
...
bool RISCV64ConstantIslands::handleConstantPoolUser(unsigned CPUserIndex) {
  CPUser &U = CPUsers[CPUserIndex];
  MachineInstr *UserMI = U.MI;
  MachineInstr *CPEMI  = U.CPEMI;
  unsigned CPI = CPEMI->getOperand(1).getIndex();
  ...
  U.CPEMI = BuildMI(NewIsland, DebugLoc(), TII->get(RISCV64::CONSTPOOL_ENTRY))
...
void RISCV64ConstantIslands::removeDeadCPEMI(MachineInstr *CPEMI) {
...
/// are zero.
bool RISCV64ConstantIslands::removeUnusedCPEntries() {
...
bool RISCV64ConstantIslands::isBBInRange
...
bool RISCV64ConstantIslands::fixupImmediateBr(ImmBranch &Br) {
...
bool
RISCV64ConstantIslands::fixupUnconditionalBr(ImmBranch &Br) {
  MachineInstr *MI = Br.MI;
  MachineBasicBlock *MBB = MI->getParent();
  ...
  if (isBBInRange(MI, DestBB, BimmX16MaxDisp)) {
    ...
    MI->setDesc(TII->get(RISCV64::BimmX16));
  }
  else {
    ...
    Br.MaxDisp = ((1<<24)-1) * 2;
    MI->setDesc(TII->get(RISCV64::JalB16));
  }
...
bool
RISCV64ConstantIslands::fixupConditionalBr(ImmBranch &Br) {
  MachineInstr *MI = Br.MI;
  unsigned TargetOperand = branchTargetOperand(MI);
  MachineBasicBlock *DestBB = MI->getOperand(TargetOperand).getMBB();
...
void RISCV64ConstantIslands::prescanForConstants() {
  unsigned J = 0;
  (void)J;
      ...
      switch(I->getDesc().getOpcode()) {
        case RISCV64::LwConstant32: {
          ...
          if (Literal.isImm()) {
            ...
            LLVM_DEBUG(dbgs() << "constant island constant " << *I << "\n");
            I->setDesc(TII->get(RISCV64::LwRxPcTcp16));
            I->RemoveOperand(1);
            I->RemoveOperand(1);
...
FunctionPass *llvm::createRISCV64ConstantIslandPass() {
  return new RISCV64ConstantIslands();
}
