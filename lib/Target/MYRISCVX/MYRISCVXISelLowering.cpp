...
lowerFRAMEADDR(SDValue Op, SelectionDAG &DAG) const {
  ...
  LLVM_DEBUG(dbgs() << "MYRISCVXTargetLowering::lowerFRAMEADDR is called\n");
...
MachineBasicBlock *MYRISCVXTargetLowering::emitAtomicBinary(
  ...
  DebugLoc DL = MI.getDebugLoc();
  unsigned LR_W, SC_W, AND, XOR, ZERO, BEQ;
  ...
  //    sc.w success, storeval, 0(ptr)
  //    beq success, $0, loopMBB
  BB = loopMBB;
  BuildMI(BB, DL, TII->get(LR_W), OldVal).addReg(Ptr);
  if (Nand) {
  ...
  BuildMI(BB, DL, TII->get(SC_W), Success).addReg(Ptr).addReg(StoreVal);
...
MachineBasicBlock *MYRISCVXTargetLowering::emitAtomicBinaryPartword(
  ...
  BB = loopMBB;
  unsigned LR_W = MYRISCVX::LR_W;
  BuildMI(BB, DL, TII->get(LR_W), OldVal).addReg(AlignedAddr);
  if (Nand) {
  ...
  unsigned SC_W = MYRISCVX::SC_W;
  BuildMI(BB, DL, TII->get(SC_W), Success)
    .addReg(StoreVal).addReg(AlignedAddr);
...
MachineBasicBlock * MYRISCVXTargetLowering::emitAtomicCmpSwap(MachineInstr &MI,
  ...
  const TargetInstrInfo *TII = Subtarget.getInstrInfo();
  DebugLoc DL = MI.getDebugLoc();
  unsigned LR_W, SC_W, ZERO, BNE, BEQ;
  ...
  BB = loop1MBB;
  BuildMI(BB, DL, TII->get(LR_W), Dest).addReg(Ptr);
  BuildMI(BB, DL, TII->get(BNE))
    .addReg(Dest).addReg(OldVal).addMBB(exitMBB);

  // loop2MBB:
  //   sc.w success, newval, 0(ptr)
