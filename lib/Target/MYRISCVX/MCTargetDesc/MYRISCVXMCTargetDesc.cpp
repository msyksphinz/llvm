...
static MCRegisterInfo *createMYRISCVXMCRegisterInfo(const Triple &TT) {
  ...
  // xxx : I don't know why it is needed.
  // InitMYRISCVXMCRegisterInfo(X, MYRISCVX::SW); // defined in MYRISCVXGenRegisterInfo.inc
  return X;
}
...
extern "C" void LLVMInitializeMYRISCVXTargetMC() {
  for (Target *T : {/* &TheMYRISCVXTarget, */ &TheMYRISCVXelTarget}) {
