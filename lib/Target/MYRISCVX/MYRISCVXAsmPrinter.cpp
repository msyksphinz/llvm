...
void MYRISCVXAsmPrinter::printSavedRegsBitmask(raw_ostream &O) {
  ...
  unsigned CPURegSize = TRI->getRegSizeInBits(MYRISCVX::CPURegsRegClass) / 8;
...
const char *MYRISCVXAsmPrinter::getCurrentABIString() const {
  switch (static_cast<MYRISCVXTargetMachine &>(TM).getABI().GetEnumValue()) {
    case MYRISCVXABIInfo::ABI::LP64:  return "abilp64";
...
extern "C" void LLVMInitializeMYRISCVXAsmPrinter() {
  // RegisterAsmPrinter<MYRISCVXAsmPrinter> X(TheMYRISCVXTarget);
  RegisterAsmPrinter<MYRISCVXAsmPrinter> Y(TheMYRISCVXelTarget);
}
